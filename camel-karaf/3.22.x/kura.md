...
Keep in mind that `KuraRouter` implements the `org.osgi.framework.BundleActivator` interface, so you need to register its `start` and `stop` lifecycle methods while [creating Kura bundle component class](http://eclipse.github.io/kura/doc/hello-example.html#create-component-class).
