...
Added `CamelContext` as first parameter to the `run` method(s) in `org.apache.camel.support.task.BlockingTask`. The `org.apache.camel.support.task.BackgroundTask` is changed to use a fixed delay between runs, which is similar to `BackOffTimer`.
...
`initialReplyIdTimeout` is now deprecated and it will be removed in a future release. Pub/sub replay ids are no longer checked at startup: when the client subscribes, if the replay id is invalid, an error is raised. To handle the error you can use a specific `ExceptionHandler` or you can set `fallbackToLatestReplayId` to `true` in order to make the consumer start from the latest event.
