...
Using `camel run` and `camel export` will only accept YAML DSL files that has naming convention `xxx.yaml` or `xxx.camel.yaml`. This allows to include resource files such as `prompt.ai.yaml` that will not be loaded as YAML DSL by Camel.
