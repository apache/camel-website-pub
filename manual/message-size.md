...
> **Important**
> Streaming message bodies (such as `InputStream`) are only supported when [Stream Caching](stream-caching.md) is enabled. Without stream caching, the body size of streaming payloads cannot be determined and will be reported as -1 (unknown).
