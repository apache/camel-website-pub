...
You can run a route hosted on GitHub using Camels [GitHub](../components/4.18.x/others/resourceresolver-github.md) resource loader.
...
We can send a message where the payload is loaded from a file (`_payload.json_`). You can also specify the payload in the CLI argument, but it’s cumbersome to specify JSON structure so often it’s better to refer to a local file.
...
When using `stream:in` to read data from _System in_ then the [Stream component](../components/4.18.x/stream-component.md) works in two modes:
...
Since Camel **4.0.0**, when using [XML DSL](../components/4.18.x/others/java-xml-io-dsl.md), we can declare additional beans in a similar way as in [YAML DSL](../components/4.18.x/others/yaml-dsl.md). Such beans will be added to the [Registry](registry.md) and can be referred to, for example, from routes.
...
Since Camel **4.0.0**, when using [XML DSL](../components/4.18.x/others/java-xml-io-dsl.md), we can also declare _beans_ using Spring Beans XML namespace. All these beans will be added to the [Registry](registry.md).
...
[XML DSL](../components/4.18.x/others/java-xml-io-dsl.md) allows using XML documents that define routes, rests and route templates. Since Camel **4.0.0** these documents may use a new root XML element (either `<camel>` or `<beans>` to resemble Spring XML DSL), where other Camel elements (like `<routes>`) are contained.
...
What’s more, we can declare some additional beans that can affect internal mechanics of `CamelContext` being run. `org.apache.camel.main.DefaultConfigurationConfigurer` is used by [Camel Main](../components/4.18.x/others/main.md) to configure `CamelContext` using beans found in [Camel Registry](registry.md).
