...
| `--open-telemetry-agent-export` | Where to export OpenTelemetry Agent traces: tui (embedded receiver in TUI) or jaeger (external Jaeger). With jaeger, start Jaeger first via 'camel infra run jaeger' and view traces at http://localhost:16686 | tui | String |
