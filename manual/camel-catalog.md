...
People, and AI agents driving Camel through tooling, often refer to a component by the protocol or product it connects to rather than by its Camel scheme: _mqtt_ (the component is `paho-mqtt5`), _s3_ (`aws2-s3`), _rabbitmq_ (`spring-rabbitmq`), _pubsub_ (`google-pubsub`). The catalog can look components, data formats and languages up by such a term, ranking the best matches first: the exact scheme or name, then declared aliases, alternative schemes, words of the title, words of the scheme or name, and finally names containing the term. A component that shares one implementation under several schemes (such as `imap`, `pop3` and `smtp`) is returned once.
...
Component developers can declare the shorthands a title cannot provide with the `aliases` attribute of `@Metadata` on the endpoint class, as the mail component does for _mail_ and _email_:
...
The catalog also includes the query options and response schema for every [Dev Console](camel-console.md), so tooling can discover what a console accepts and what it returns without having to start a running Camel application first.
