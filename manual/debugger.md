...
The Debugger allows tooling or the likes to attach breakpoints which are being invoked when [Exchanges](exchange.md) are routed.
...
![image](_images/images/debug.png)
...
There is also a [Backlog Debugger](backlog-debugger.md) that allows debugging from JMX. It is automatically provided when `camel-debug` is on the classpath (since 3.16) or when using the `camel:debug` Maven goal (since 3.18).
