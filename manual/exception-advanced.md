...
[Dead Letter Channel](../components/4.22.x/eips/dead-letter-channel.md) has support for **`onRedelivery`** to allow custom processing of a Message before its being redelivered. It can be used to add some customer header or whatnot. In Camel 2.0 we have added this feature to [Exception Clause](exception-clause.md) as well, so you can use per exception scoped on redelivery. Camel will fallback to use the one defined on [Dead Letter Channel](../components/4.22.x/eips/dead-letter-channel.md) if any, if none exists on the [Exception Clause](exception-clause.md). See [Dead Letter Channel](../components/4.22.x/eips/dead-letter-channel.md) for more details on **`onRedelivery`**.
