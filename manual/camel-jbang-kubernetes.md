...
Prerequisites: [kubectl](https://kubernetes.io/docs/tasks/tools/#kubectl) installed, connected to a Kubernetes cluster (remote, [Kind](https://kind.sigs.k8s.io/docs/user/quick-start/), or [Minikube](https://minikube.sigs.k8s.io/docs/start/)).
...
| `--profile` | The developer [profile](camel-jbang-running.html#_using_profiles) to use a specific configuration in configuration files using the naming style `application-<profile>.properties`. |
> **Tip**
...
Generate a [CronJob](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/) instead of a Deployment (requires `enabled=true` and `schedule`):
...
Use [Knative serving](https://knative.dev/docs/serving/) for serverless workloads with auto-scaling and scale-to-zero.
...
Configure the Camel Knative component to interact with [Knative eventing](https://knative.dev/docs/eventing/) and [Knative serving](https://knative.dev/docs/serving/). The export auto-configures both the component and the Kubernetes manifest.
