...
The option `backlogTracing=true` now automatic enabled the tracer on startup. The previous behavior was _surprisingly_ that the tracer was only made available, and had to be manually enabled afterwards. The old behavior can be archived by setting `backlogTracingStandby=true`.
...
Then calling `_myservice_` would before require including the context-path, such as `[http://localhost:8080/camel/myservice](http://localhost:8080/camel/myservice)`. Now the context-path is not in use, and the endpoint can be called with `[http://localhost:8080/myservice](http://localhost:8080/myservice)`.
