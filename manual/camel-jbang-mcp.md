...
The server is built on [Quarkus](https://quarkus.io/) using the [quarkus-mcp-server](https://docs.quarkiverse.io/quarkus-mcp-server/dev/index.md) extension and ships as a single uber-JAR that can be launched via [JBang](https://www.jbang.dev/).
...
> **Important**
> The MCP server does **not** expose REST endpoints. You cannot query tools or resources with plain HTTP GET requests (e.g., `curl [http://localhost:8080/mcp/tools](http://localhost:8080/mcp/tools)` will return a 404). All communication uses JSON-RPC messages over the MCP protocol.
...
```bash
java -Dquarkus.http.host-enabled=true -Dquarkus.http.port=8080 -jar dsl/camel-jbang/camel-jbang-mcp/target/camel-jbang-mcp-4.19.0-SNAPSHOT-runner.jar
```
...
Open the Inspector UI at [http://localhost:6274/](http://localhost:6274/) and configure the connection:
