...
| `camel_write_file` | Writes the complete content of a file in the project directory. YAML and `.properties` content is validated first; invalid content is not written and the errors are returned. Only a plain file name in the directory is accepted. Nobody is asked before the write: the MCP client (Claude Code, Cursor and the others ask before a tool that is not read-only runs) is where the human sits, and the `read-only` access level of the security layer hides the tool altogether. |
| `camel_edit_file` | Changes a file by replacing one snippet: the lines to find (they must name one place; other indentation is fine) and what to put there. Validated and reloaded as a write is, and the answer says at which line it changed. A model that rewrites a whole file to add one step corrupts the lines it did not mean to touch, so this is the tool for a change to an existing file; `camel_write_file` writes a new one. |
...
| `camel_eval_expression` | Evaluates an expression (simple by default) in the running integration, or locally when none is named, and returns the value (true/false for a predicate) or the syntax error with the position where it breaks, so an agent can check an expression before writing it into a route. Any language works: one that is not on the server’s own classpath, `jsonpath`, `jq`, `xpath`, `groovy`, has its component downloaded on first use and the answer says so in `downloaded`. |
...
| `camel_runtime_circuit_breakers` | Circuit breaker state (CLOSED/OPEN/HALF\_OPEN), call counts, failure rates and not-permitted calls. |
