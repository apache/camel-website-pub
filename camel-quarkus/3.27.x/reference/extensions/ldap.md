...
-   [LDAP component](../../../../components/4.14.x/ldap-component.md), URI syntax: `ldap:dirContextName`
...
When using a custom `SSLSocketFactory` in native mode, such as the one in the [Configuring SSL](../../../../components/4.14.x/ldap-component.html#_configuring_ssl) section, you need to register the class for reflection otherwise the class will not be made available on the classpath. Add the `@RegisterForReflection` annotation above the class definition, as follows:
