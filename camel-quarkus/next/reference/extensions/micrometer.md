...
-   [Micrometer component](../../../../components/4.22.x/micrometer-component.md), URI syntax: `micrometer:metricsType:metricsName`
...
Comma separated list of route IDs to exclude from metrics collection.
 ...
 | `string` |  |
| `[quarkus.camel.metrics.log-metrics-on-shutdown](#quarkus-camel-metrics-log-metrics-on-shutdown)`

Log metrics when application is shutting down. (default, `false`).

 | `boolean` | `false` |
| `[quarkus.camel.metrics.log-metrics-on-shutdown-filters](#quarkus-camel-metrics-log-metrics-on-shutdown-filters)`

List of metrics (comma separated) to log when application is shutting down. You can use **character to log any metrics containing the wildcard, for example `camel.exchanges.`** (default to all metrics available).
