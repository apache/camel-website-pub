...
This part should be relatively minimal for Camel applications. Most of it is automated by an [OpenRewrite recipe](https://docs.openrewrite.org/recipes/quarkus/spring/springboottoquarkus).
...
If OpenRewrite recipe used, it will have the @QuarkusMain annotation, otherwise the @SpringBootApplication. This class can be completely removed. if not removed, the application will start but won’t discover and start the Camel routes.
...
In case, the Camel Spring XML DSL was used, it needs to be migrated. Please see [Camel XML IO DSL documentation](../../../components/4.22.x/others/java-xml-io-dsl.md)

## Automated tests

Tests annotated with `@CamelSpringBootTest`, `@CamelSpringTest` and `@SpringBootTest` must use `@QuarkusTest` instead.
...
As a reminder and in case of specific cases, see [Camel Quarkus Testing User guide](../user-guide/testing.md).
...
The system of profiles are relatively similar. You can check the [Quarkus profile documentation](https://quarkus.io/guides/config-reference#profiles) if you have very specific cases.
...
-   [Camel Quarkus transaction guide for Red Hat product](https://docs.redhat.com/en/documentation/red_hat_build_of_apache_camel/4.14/html/red_hat_build_of_apache_camel_for_quarkus_reference/ass-ceq-jta-transaction-guide)
    
-   [Quarkus transaction guide](https://quarkus.io/guides/transaction)
    
-   [Camel transaction guide which is focused on Spring](../../../components/4.22.x/eips/transactional-client.md)
    

## Custom components

If you have developed your custom components, you need to ensure that Jandex is provided and to configure the Camel Service discovery if there is one. See [this tutorial for details](https://jamesnetherton.github.io/blog/2023-09-15-how-to-use-custom-components-with-camel-quarkus)

To go native, you will need to follow the [Quarkus guide to write native extension](https://quarkus.io/guides/writing-extensions).
