...
Here, just a sketch of the repeating part, after you have performed the initial setup following the [Camel Release guide](../../../manual/release-guide.md).

## Pre-release tasks

For new major / minor version releases:

-   When releasing after a recent Quarkus release, check for new releases of third party Quarkus extensions we depend on. They are listed in the [Primary dependencies](https://github.com/apache/camel-quarkus/blob/main/pom.xml#L48-L61) section of the top level `pom.xml`. If there are new releases, upgrade the versions accordingly and run `mvn cq:sync-versions -N` from the root directory of the source tree. Note that some of the versions are mandated by quarkus, so camel-quarkus should use the same values as the quarkus-platform bom.
    
-   Close the GitHub release [milestone](https://github.com/apache/camel-quarkus/milestones) and assign any remaining issues that aren’t covered by the release to the next milestone
...
-   Login to the [Apache staging repository](https://repository.apache.org) using your Apache LDAP credentials.
...
This will take care of fetching the release source zip and SBOMs from [https://repository.apache.org](https://repository.apache.org), creating SHA512 checksums and pushing the content to ASF `dist/dev/`

After the changes have been committed. You can check them by browsing to [https://dist.apache.org/repos/dist/dev/camel/camel-quarkus/](https://dist.apache.org/repos/dist/dev/camel/camel-quarkus/).
...
Copy/paste the generated report into issue [CAMEL-QUARKUS-4012](https://github.com/apache/camel-quarkus/issues/4012).
...
Send a mail to start the upstream vote. There are plenty of mails on the dev mailing list to get [inspiration](https://lists.apache.org/thread/ntjrw585l5mjtrsvd6sn530boh43ccqo) from. For instance, one could search mail with subject below:
...
After at least 72 hours has past, close the vote. There are plenty of mails on the dev mailing list to get [inspiration](https://lists.apache.org/thread/shyfw2ltrxknnzb26nf71bc406lxvsp4) from. For instance, one could search mail with subject:
...
Ideally, the release announcement should be written before submitting the [Platform pull request](#quarkus-platform) so that Quarkus team can mention our release in their announcement. In that way the information will reach the whole Quarkus audience.

Please, keep in mind a few things when preparing the pull request for the camel-website:

-   A release marker file should be added in [https://github.com/apache/camel-website/tree/main/content/releases/q](https://github.com/apache/camel-website/tree/main/content/releases/q)
...
-   Documentation branches for LTS releases should be updated in [antora-playbook.yml](https://github.com/apache/camel-website/blob/main/antora-playbook-snippets/antora-playbook.yml)
...
You can proceed with upgrading Camel Quarkus in Quarkus Platform once the newly released artifacts are available on [Maven Central](https://repo1.maven.org/maven2/org/apache/camel/quarkus/camel-quarkus-bom/).
...
> **Note**
> [Quarkus Platform](https://github.com/quarkusio/quarkus-platform) hosts the metadata and Maven BOMs necessary for [code.quarkus.io](https://code.quarkus.io/) as well as for [Quarkus tools](https://quarkus.io/guides/tooling).
...
-   Change `camel-quarkus.version` property in the Quarkus platform top level `[pom.xml](https://github.com/quarkusio/quarkus-platform/blob/main/pom.xml#L54)` to the newly released version:
...
-   Paste the URL of the [Camel Quarkus release announcement](#release-announcement) to the description field of the pull request. Quarkus team will then be able to refer to our release announcement from their release announcement. The online availability of our release announcement should not block submitting the PR. You can paste the link even before the content is online, but make sure that it is available at the latest by the time when Quarkus team will publish their announcement.
    
-   Optionally, if there are some extraordinary new features, fixes, etc., you can tag the pull request with `release/noteworthy-feature` label and provide a short text about those features in the PR description that Quarkus team will pick into their announcement.
    

## Create a GitHub release

This will trigger sending a notification to folks watching the Camel Quarkus GitHub repository, so it should ideally happen once the newly released artifacts are available on [Maven Central](https://repo1.maven.org/maven2/org/apache/camel/quarkus/camel-quarkus-bom/).
...
This section needs to be executed only when a Quarkus Platform release has been published. It should be the case most of the time. In this case, right after the newest Quarkus Platform becomes available on [Maven Central](https://repo1.maven.org/maven2/io/quarkus/platform/quarkus-bom/):

> **Note**
> For patch releases, the instructions that follow are mostly the same. Except that you must `git checkout` the desired maintenance branch (E.g 3.20.x), perform any updates on there and send PRs to the same upstream maintenance branch. There is no need to interact with the `camel-quarkus-main` branch in this case.

-   Make sure all [PRs](https://github.com/apache/camel-quarkus-examples/pulls) against `camel-quarkus-main` branch are merged.
...
-   [Create a ticket](https://github.com/apache/camel-quarkus/issues/new) asking a [PMC member](https://projects.apache.org/committee.html?camel) to update the [Apache Committee Report Helper](https://reporter.apache.org/addrelease.html?camel). The ticket title could be as follows. Release: The Apache Committee Report Helper should be updated by a PMC member as camel-quarkus-X.Y.Z has been released on YYYY-MM-DD.
...
\[ERROR\] fatal: could not read Username for 'https://gitbox.apache.org': terminal

In some environments, an SSH askpass helper may be invoked unexpectedly for credential handling. If the helper is missing or incompatible with the desktop/session configuration, Git operations can fail. One way to prevent the use of an askpass helper is to disable it explicitly:

unset SSH\_ASKPASS
export SSH\_ASKPASS\_REQUIRE=never

After applying these settings, the error may change to:

\[ERROR\] The git-push command failed.
\[ERROR\] Command output:
\[ERROR\] fatal: could not read Username for 'https://gitbox.apache.org': terminal prompts disabled
\[ERROR\] -> \[Help 1\]

At this point, Git is no longer attempting to use an askpass helper, but is also unable to prompt for credentials interactively. To restore normal credential prompting or authentication behavior, several commands may be helpful, depending on the environment and Git configuration. The following commands were executed during troubleshooting; the exact one that resolved the issue is unclear, but they are documented here for reference:

ssh -T git@gitbox.apache.org
