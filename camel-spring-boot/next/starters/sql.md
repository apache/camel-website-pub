...
| --- | --- | --- | --- |
| camel.component.sql-stored.allow-template-from-header | Whether to allow overriding the endpoint-configured stored-procedure template with the CamelSqlStoredTemplate header. Disabled by default; a header-supplied template is resolved with placeholders only, never as a file:/http: resource. This is the default for all sql-stored endpoints and can be overridden per endpoint. | false | Boolean |
