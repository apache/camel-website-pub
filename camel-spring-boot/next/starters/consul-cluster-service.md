...
This starter provides leader election and cluster coordination for Camel routes using [HashiCorp Consul](https://www.consul.io) as the distributed backend. It integrates with Camel’s `CamelClusterService` SPI so you can use the clustered route controller to ensure only one instance of a route runs across a cluster.
