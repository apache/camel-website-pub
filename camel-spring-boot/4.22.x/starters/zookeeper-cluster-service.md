...
This starter provides leader election and cluster coordination for Camel routes using [Apache ZooKeeper](https://zookeeper.apache.org) as the distributed backend. It integrates with Camel’s `CamelClusterService` SPI so you can use the clustered route controller to ensure only one instance of a route runs across a cluster.
...
| camel.cluster.zookeeper.reconnect-max-sleep-time | Max time to sleep on each retry. Default is Integer.MAX\_VALUE. |  | Long |
