...
| camel.component.ssh.fail-on-unknown-host | Specifies whether a connection to an unknown SSH server host key should fail. When false, host keys that are not present in the known\_hosts resource are accepted. | false | Boolean |
...
| camel.component.ssh.known-hosts-resource | Sets the resource path for a known\_hosts file used to verify the SSH server host key. When not set, the client does not verify the server host key against a known\_hosts file. |  | String |
