...
| camel.component.http.proxy-auth-scheme | Proxy server connection protocol scheme. Defaults to http regardless of the target endpoint scheme, because most corporate HTTP proxies expect a plain HTTP connection on their listener port. | http | String |
