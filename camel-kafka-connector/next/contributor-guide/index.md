...
-   Try the [Getting started](../user-guide/index.md) guides
    
-   Digging in the codebase and tune some operations or add new features.
    
-   Take a look at the [open issues](https://github.com/apache/camel-kafka-connector/issues) and leave a comment on the issue to let us know you are working on it.
...
We can also be reached on the Zulip chat at [https://camel.zulipchat.com](https://camel.zulipchat.com).

We track issues using the [issues tracker in Github](https://github.com/apache/camel-kafka-connector/issues)

When you’re ready to contribute create a Pull request to the [camel-kafka-connector repository](https://github.com/apache/camel-kafka-connector/)
...
The first step is to create an account on Salesforce. The service allows developers to create a free account for testing. The account can be created on their [login](https://login.salesforce.com/) website.

The next step is to create a new connected application. This will provide you with the API keys that allow the automation to run. The [Salesforce help](https://help.salesforce.com/articleView?id=connected_app_create_api_integration.htm) contains the documentation and steps related to this part of the process.

After you create the API keys proceed to adjust a few more parameters necessary for the tests to run. Specifically, the [IP Relaxation/range policy](https://help.salesforce.com/articleView?err=1&id=connected_app_overview.htm&type=5) parameter should be adjusted (usually to the value "Relax IP restrictions").

You need setup change data capture to allow our tests to read from the Account table. The [Change Data Capture Developer Guide](https://developer.salesforce.com/docs/atlas.en-us.change_data_capture.meta/change_data_capture/cdc_intro.htm) on the Salesforce documentation is the recommended starting point for this.
...
A few approaches to leverage this available processing power have emerged recently. One of them works particularly well with this project: [mvnd - The Maven Daemon](https://github.com/mvndaemon/mvnd). The project embeds Maven itself to leverage its native parallelization and implement lower level build optimizations. It can greatly reduce the build time:
