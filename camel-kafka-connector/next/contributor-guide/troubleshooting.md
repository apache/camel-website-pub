...
-   [Kafka Connect does not load the Camel connector](#connector-not-loaded)
...
A common root cause for Kafka Connect failing to serialize or deserialize the data in transit is incorrect values for the [key.converter](https://kafka.apache.org/documentation/#key.converter) or the [value.converter](https://kafka.apache.org/documentation/#value.converter) (or both). There are usually verifiable in the logs by looking for `Caused by: org.apache.kafka.common.errors.SerializationException` or `Caused by: org.apache.kafka.connect.errors.DataException`.
...
When using append chars, it must be UTF-8 encoded so that they can be transmitted over the internet (i.e.: for new line use use %0A instead of \\n and so on). You can check [the W3 Schools encoding reference](https://www.w3schools.com/tags/ref_urlencode.ASP) for a reference list of characters and their encoded strings.