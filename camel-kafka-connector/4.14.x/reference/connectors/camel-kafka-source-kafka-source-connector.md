...
Connector Description: Receive data from Kafka topics through Plain Login Module.
...
The camel-kafka-source source connector supports 13 options, which are listed below.
...
| **camel.kamelet.kafka-source.securityProtocol** | Protocol used to communicate with brokers. SASL\_PLAINTEXT, PLAINTEXT, SASL\_SSL and SSL are supported. | "SASL\_SSL" | MEDIUM |
| **camel.kamelet.kafka-source.saslMechanism** | The Simple Authentication and Security Layer (SASL) Mechanism used. | "PLAIN" | MEDIUM |
| **camel.kamelet.kafka-source.user** | **Required** Username to authenticate to Kafka. |  | HIGH |
| **camel.kamelet.kafka-source.password** | **Required** Password to authenticate to kafka. |  | HIGH |
...
| **camel.kamelet.kafka-source.pollOnError** | What to do if kafka threw an exception while polling for new messages. There are 5 enums and the value can be one of `DISCARD`, `ERROR_HANDLER`, `RECONNECT`, `RETRY`, `STOP`. | "ERROR\_HANDLER" | MEDIUM |
