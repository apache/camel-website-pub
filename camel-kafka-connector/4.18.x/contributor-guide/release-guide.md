...
Here, just a sketch of the repeating part, after you have performed the initial setup following the [Camel Release guide](../../../manual/release-guide.md). In particular the section about setting up your maven environment as described in [Camel Release Maven Setup](../../../manual/release-guide.html#ReleaseGuide-MavenSetup).

## Pre release tasks

-   Close the GitHub release [milestone](https://github.com/apache/camel-kafka-connector/milestones) and assign any remaining issues that aren’t covered by the release to the next milestone
...
> **Important**
> If you followed [Camel Release Maven Setup](../../../manual/release-guide.html#ReleaseGuide-MavenSetup) you should have your gpg mave settings in a profile called `gpg` tha is used in some commands as `-Pgpg`. If that is not the case, change the commands accordingly to your setup.
...
> **Tip**
> All the steps above have been automated in a script that you can run with `./release-utils/release.sh` . You should input the requested information that are used to fill in the exported variable describe in xref:\_export\_local\_variables\_for\_convenience section.
...
-   Go to [https://repository.apache.org](https://repository.apache.org)
...
Send an email to [dev@camel.apache.org](mailto:dev@camel.apache.org) with object: `[VOTE] Release Camel-Kafka-connector $RELEASE_VERSION` you can select 48 or 72 hours for the vote. The message can be something like:
...
> **Note**
> The version you would usually use is the branch one, i.e. 4.8.x if you are releasing 4.8.0 or subsequent patch versions.
...
Create a release note in [https://github.com/apache/camel-website/tree/master/content/releases/ckc](https://github.com/apache/camel-website/tree/master/content/releases/ckc) by coping and changing one of the already present.

## Update Antora playbook

Add the created release branch to [https://github.com/apache/camel-website/blob/main/antora-playbook-snippets/antora-playbook.yml](https://github.com/apache/camel-website/blob/main/antora-playbook-snippets/antora-playbook.yml) under:
