...
Published 2024-01-12 by Tadayoshi Sato in Releases, Tooling

Hawtio 3.0.0 GA is released. What's new?
...
The Camel JBang CLI already has a nice `camel hawtio` command. [2](#fn:2) We can expect that a future update will make Hawtio v3 default for the command, but meanwhile if it is still using the old v2 console, you can make it start the new Hawtio v3 from the CLI easily as follows: [3](#fn:3)
...
Once executed, a browser should automatically open and the Hawtio console should be visible at [http://localhost:8888/hawtio/](http://localhost:8888/hawtio/). Select the `Discover` tab in the `Connect` plugin. This will then discover a locally running Camel route with the Jolokia agent attached.
...
-   [https://github.com/hawtio/hawtio-sample-plugin-ts](https://github.com/hawtio/hawtio-sample-plugin-ts) (for Spring Boot)
-   [https://github.com/hawtio/hawtio-sample-war-plugin-ts](https://github.com/hawtio/hawtio-sample-war-plugin-ts) (for WAR deployments)

## Next step

If you are interested in trying Hawtio v3, please start by reading Get Started in the official documentation.

-   [Hawtio - Get Started](https://hawt.io/docs/get-started.md)
...
1.  Such as AngularJS 1.x and PatternFly 3.x. [↩︎](#fnref:1)
    
2.  [Camel JBang - Using Jolokia and Hawtio](../../../../manual/camel-jbang-managing.html#_using_jolokia_and_hawtio) [↩︎](#fnref:2)
    
3.  3.0.0 has a problem with `camel hawtio`, so please use 3.0.1. [↩︎](#fnref:3)