...
Published 2025-02-13 by Tadayoshi Sato in AI

Discover how to use Camel TorchServe component to effortlessly run AI model inference on TorchServe within your Camel application

## Introduction

In the just released Apache Camel 4.10 LTS, AI-related components have been further enhanced. Among others, three new components related to AI model serving have been added. [1](#fn:1)

-   TorchServe component
-   TensorFlow Serving component
-   KServe component

My previous article [Apache Camel AI: Leverage power of AI with DJL component](../../../../blog/2024/09/camel-ai-examples/index.md) demonstrated how the DJL component can be used to perform AI model inference within the Camel routes. Starting from 4.10, in addition to the in-route inference by DJL, these new components will allow the Camel routes to invoke external model servers to perform inference.
...
[TorchServe](https://pytorch.org/serve/index.md) is the serving feature provided by the popular machine learning framework PyTorch. The Camel TorchServe component allows you to call the [REST API](https://pytorch.org/serve/rest_api.md) served by the TorchServe server from within the Camel routes.
...
First, let’s install [Camel CLI](../../../../manual/camel-jbang.md) if it is not installed yet:

* * *

_**INFO:** If JBang is not installed, first install JBang by referring: [https://www.jbang.dev/download/](https://www.jbang.dev/download/)_
...
The readiness of the TorchServe server can be checked by accessing [http://localhost:8080/ping](http://localhost:8080/ping), but it can also be checked from the Camel route using the TorchServe component.
...
TorchServe has [Management API](https://pytorch.org/serve/management_api.md) from which you can manage models corresponding to steps 1, 2 and 4 in [Basic usage](#basic-usage). Camel TorchServe component also supports this management API and allows you to perform the respective management operations from the Camel routes.
...
Let’s register a model using Camel. Here we will use a pre-trained MNIST V2 model provided at the [PyTorch Model Zoo](https://pytorch.org/serve/model_zoo.md).
...
Finally, it is also worth mentioning TorchServe’s [Metrics API](https://pytorch.org/serve/metrics_api.md), which outputs model usages as Prometheus-style metrics.
...
[https://github.com/megacamelus/camel-ai-examples](https://github.com/megacamelus/camel-ai-examples)

The next article will introduce the remaining two serving components. Stay tuned!

* * *

1.  Camel TorchServe component has been provided since 4.9. [↩︎](#fnref:1)