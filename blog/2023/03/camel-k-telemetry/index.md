...
Published 2023-03-09 by Gaëlle Fournier in Camel K, Howtos

Camel K: how to configure Distributed Tracing for an Integration
...
For more information see the [documentation](../../../../camel-k/next/installation/installation.md)
...
If Jaeger is not already available on your kubernetes instance, an easy way to install it on a kubernetes is to install the Jaeger operator (see [https://www.jaegertracing.io/docs)](https://www.jaegertracing.io/docs%29).
...
Now that we have Jaeger available on the Kubernetes cluster, the next step is to run some integrations in a Kubernetes cluster where Camel K is installed. You can find the source code in the [Camel K example GitHub repository](https://github.com/apache/camel-k-examples/tree/main/generic-examples/traits/telemetry).

We will be creating two integration routes as the distributed services:

-   _Inventory_ which is also a rest service in charge of product’s inventory management
...
-   _Order_ which is a rest service called by a user to place an order for a product, that will then notify the _Inventory_ service
...
2.  Now, we will stop collecting the traces for the _Order_ integration:
...
Let’s deploy two different _Order_ integrations to compare the traces generated :

-   stop collecting the traces from _Inventory_ integration
...
-   update the _Order_ integration using the `telemetry` trait
...
-   the _Order_ integration using the `tracing` trait
