...
Published 2026-02-19 by Claus Ibsen, Andrea Cosentino, Ivo Bek, Federico Mariani, Claudio Miranda, Luigi De Masi in Releases

Details of what we have done in the Camel 4.18 LTS release.

Apache Camel 4.18 LTS has just been [released](../../../../blog/2026/02/RELEASE-4.18.0/index.md).
...
We added a small example with a custom function [stock function](https://github.com/apache/camel-kamelets-examples/tree/main/jbang/simple-custom-function), which returns a stock value which can be with Camels simple language with `${stock()}` as shown below:
...
-   OpenSSH compliance: Strict known\_hosts semantics
...
We introduced the **Camel MCP Server**, a [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server that exposes the Apache Camel Catalog and a set of specialized tools to AI coding assistants such as Claude Code, OpenAI Codex, GitHub Copilot, and JetBrains AI.

The server is built on Quarkus using the [quarkus-mcp-server](https://docs.quarkiverse.io/quarkus-mcp-server/dev/index.md) extension and ships as a single uber-JAR that can be launched via [JBang](https://www.jbang.dev/). It supports both **STDIO** (default, for CLI-based AI tools) and **HTTP/SSE** transports.
...
This module is in **Preview** status as of Camel 4.18. For a deeper dive, see the dedicated [blog post](../../../../blog/2026/02/camel-jbang-mcp/index.md).
...
When using `camel-main` or `camel-quarkus` then Rest DSL in _contract first_ style is now registering each API endpoints individually with the underlying HTTP server which allows Quarkus to register access control and metrics per endpoint instead of a single combined overall endpoints as previously. This now works the same way as Rest DSL in _code first_ style behaves.
...
Make sure to read the [upgrade guide](../../../../manual/camel-4x-upgrade-guide-4_18.md) if you are upgrading from a previous Camel version.
