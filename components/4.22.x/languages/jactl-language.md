...
Camel has support for using [Jactl](https://jactl.io).
...
The Jactl language supports the following options which are listed below.
...
1.  The classname of the runtime type (e.g. "com.acme.ApplicationContext") of the object is passed to the predicate to see if we are allowed to invoke methods on it. The predicate should return true if it is an allowed class.
    
2.  The class where the method is declared is passed to the predicate to make sure that not only the object’s class is permitted but the class in which the method is implemented is also permitted (this could be the same class, or it could be a base class if the method is inherited).
...
For more control over configuration of the `Jactl` language, pass your own [JactlContext](https://jactl.io/docs/integration-guide/jactl-context) to the `JactlLanguage` constructor:
