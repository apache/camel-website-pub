...
Bucket-level operations (`listBuckets`, `deleteBucket`, `listObjects`, `createBucket`, etc.) don’t require a key header — just set the `operation` parameter.
...
| --- | --- | --- |
| `listBuckets` | List all buckets for this account in this region. |  |
| `createBucket` | Create a new bucket. |  |
| `deleteBucket` | Delete a bucket. |  |
| `headBucket` | Check if a bucket exists and you have permission to access it. |  |
| `listObjects` | List objects in a bucket. |  |
| `getObject` | Get a single object. Returns an `S3Object` instance. |  |
| `getObjectRange` | Get a byte range from an object. | See [getObjectRange](#getObjectRange) |
| `deleteObject` | Delete an object. |  |
| `deleteObjects` | Delete multiple objects in a single request. | See [deleteObjects](#deleteObjects) |
| `copyObject` | Copy an object from one bucket to another. | See [copyObject](#copyObject) |
| `headObject` | Retrieve metadata from an object without returning the object itself. |  |
| `uploadObject` (default) | Upload a file to S3 based on the body content. | See [Upload (Single Upload)](#uploadObject) |
| `multipartUpload` | Upload a large file using multipart upload. | See [Multipart Upload](#multipartUpload) |
| `createDownloadLink` | Generate a presigned download URL. | See [createDownloadLink](#createDownloadLink) |
| `createUploadLink` | Generate a presigned upload URL. | Requires `accessKey`, `secretKey`, `region` |
| `restoreObject` | Restore an archived object from Glacier storage. | See [restoreObject](#restoreObject) |
| `getObjectTagging` | Get the tags of an object. |  |
| `putObjectTagging` | Set tags on an object. | Requires `Map<String, String>` via `CamelAwsS3ObjectTags` header |
| `deleteObjectTagging` | Delete all tags from an object. |  |
| `getObjectAcl` | Get the ACL of an object. |  |
| `putObjectAcl` | Set the ACL on an object. | Use `CamelAwsS3CannedAcl` header (e.g., `PublicRead`) |
| `getBucketTagging` | Get the tags of a bucket. |  |
| `putBucketTagging` | Set tags on a bucket. | Requires `Map<String, String>` via `CamelAwsS3BucketTags` header |
| `deleteBucketTagging` | Delete all tags from a bucket. |  |
| `getBucketVersioning` | Get the versioning configuration of a bucket. |  |
| `putBucketVersioning` | Set the versioning configuration of a bucket. | Use `CamelAwsS3VersioningStatus` header (`Enabled` or `Suspended`) |
| `getBucketPolicy` | Get the bucket policy as a JSON string. |  |
| `putBucketPolicy` | Set a bucket policy. | See [putBucketPolicy](#putBucketPolicy) |
