...
To work with Dropbox API, you need to obtain an **accessToken**, **expireIn**, **refreshToken**, **apiKey**, **apiSecret** and a **clientIdentifier.**  
...
The Dropbox component supports the following options which are listed below.
...
The Dropbox component supports the following message header(s), which is/are listed below:
...
| `Map<String, byte[]>` or `Map<String, CachedOutputStream>` if stream caching is enabled | in the case of multiple files downloaded, a map with as a key the path of the remote file downloaded and as value the stream representing the file downloaded |
...
| Property | Mandatory | Description |
| --- | --- | --- |
| `uploadMode` | `true` | add or force this option specifies how a file should be saved on dropbox: in the case of `add`, the new file will be renamed if a file with the same name already exists on dropbox. In the case of `force`, if a file with the same name already exists on dropbox, this will be overwritten. |
| `localPath` | `false` | Folder or file to upload on Dropbox from the local filesystem. If this option has been configured, then it takes precedence over uploading as a single file with content from the Camel message body (the message body is converted into a byte array). |
...
| `Map<String, DropboxResultCode>` | in the case of multiple files upload, a map with as a key the path of the remote file uploaded and as value the result of the upload operation, OK or KO |
...
| Object type | Description |
| --- | --- |
| `List<com.dropbox.core.v2.files.SearchMatchV2>` | list of the file path found. For more information on this object, refer to [Dropbox documentation](https://javadoc.io/doc/com.dropbox.core/dropbox-core-sdk/latest/com/dropbox/core/v2/files/SearchMatchV2.md). |