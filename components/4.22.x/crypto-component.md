...
This approach is useful when you want to apply PQC signatures directly using the `crypto:sign` / `crypto:verify` pair without the full `camel-pqc` component. For hybrid signatures (combining classical + PQC) or key lifecycle management, use the [PQC component](pqc-component.md) instead.
...
For hybrid signatures that combine a classical algorithm (e.g., ECDSA) with a PQC algorithm (e.g., ML-DSA) in a single route, the recommended approach is to use the [PQC component](pqc-component.md) which provides built-in hybrid operations.
...
See the [PQC component](pqc-component.md) for the full list of supported classical algorithms and details on the hybrid wire format.
