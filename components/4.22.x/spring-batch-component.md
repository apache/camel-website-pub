...
The Spring Batch component and support classes provide integration bridge between Camel and [Spring Batch](http://www.springsource.org/spring-batch) infrastructure.
...
The Spring Batch component supports the following options which are listed below.

   
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| **jobLauncher** (producer) | **Autowired** Explicitly specifies a JobLauncher to be used. |  | JobLauncher |
| **jobRegistry** (producer) | **Autowired** Explicitly specifies a JobRegistry to be used. |  | JobRegistry |
...
The Spring Batch component supports the following message header(s), which is/are listed below:

   
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| **CamelSpringBatchJobName** (producer) Constant: [`JOB_NAME`](https://javadoc.io/doc/org.apache.camel/camel-spring-batch/latest/org/apache/camel/component/spring/batch/SpringBatchConstants.html#JOB_NAME) | The name of the Batch Job to use. |  | String |

## Usage

When the Spring Batch component receives the message, it triggers the job execution. The job will be executed using the `org.springframework.batch.core.launch.JobLaucher` instance resolved according to the following algorithm:
...
-   if none of the steps above allow resolving the `JobLauncher` and there is exactly one `JobLauncher` instance in the Camel Registry, then use it.
