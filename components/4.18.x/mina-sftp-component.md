...
| **certBytes** (security) | Set the OpenSSH certificate as byte array. |  | byte\[\] |
| **certFile** (security) | Set the OpenSSH certificate file path for certificate-based authentication. |  | String |
| **certUri** (security) | Set the OpenSSH certificate as a classpath: or file: URI. |  | String |
...
| **keyPair** (security) | Set a java.security.KeyPair directly for public key authentication. |  | KeyPair |
...
| **preferredAuthentications** (security) | Set the preferred authentications which SFTP endpoint will used. Some example include: password,publickey. If not specified the default list from MINA SSHD will be used. |  | String |
| **privateKey** (security) | Set the private key as byte array for public key authentication. |  | byte\[\] |
| **privateKeyFile** (security) | Set the private key file path so that the SFTP endpoint can do public key authentication. |  | String |
| **privateKeyPassphrase** (security) | Set the passphrase for decrypting an encrypted private key. |  | String |
| **privateKeyUri** (security) | Set the private key as a classpath: or file: URI (e.g., classpath:keys/id\_rsa). |  | String |
| **publicKeyAcceptedAlgorithms** (security) | Set a comma separated list of public key accepted algorithms. If not specified the default list from MINA SSHD will be used. |  | String |
