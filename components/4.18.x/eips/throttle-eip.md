...
The Throttler uses an [Expression](../../../manual/expression.md) to configure the number of requests. In all the examples from above, we used a [constant](../languages/constant-language.md). However, the expression can be dynamic, such as determined from a message header from the current `Exchange`.
