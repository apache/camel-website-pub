...
The Kubernetes Deployments component is one of [Kubernetes Components](kubernetes-summary.md) which provides a producer to execute Kubernetes Deployments operations and a consumer to consume events related to Deployments objects.
...
The Kubernetes Deployments component supports 4 options, which are listed below.
...
The Kubernetes Deployments component supports 9 message header(s), which is/are listed below:
...
| **CamelKubernetesDeploymentSpec** (producer) Constant: [`KUBERNETES_DEPLOYMENT_SPEC`](https://javadoc.io/doc/org.apache.camel/camel-kubernetes/latest/org/apache/camel/component/kubernetes/KubernetesConstants.html#KUBERNETES_DEPLOYMENT_SPEC) | The spec for a deployment. |  | DeploymentSpec |
...
When using kubernetes-deployments with Spring Boot make sure to use the following Maven dependency to have support for auto configuration:
