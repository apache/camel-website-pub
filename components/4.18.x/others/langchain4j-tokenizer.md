...
The LangChain4j tokenizer component provides support to tokenize (chunk) larger blocks of texts into text segments that can be used when interacting with LLMs. Tokenization is particularly helpful when used with [vector databases](https://en.wikipedia.org/wiki/Vector_database) to provide better and more contextual search results for [retrieval-augmented generation (RAG)](https://en.wikipedia.org/wiki/Retrieval-augmented_generation).

This component uses the [LangChain4j document splitter](https://docs.langchain4j.dev/tutorials/rag/#document-splitter) to handle chunking.
...
The tokenization creates a composite message (i.e.: an array of Strings). This composite message, then can be split using the [Split EIP](../eips/split-eip.md) so that each text segment is separately to an endpoint. Alternatively, the contents of the composite message may be passed through a processor so that invalid data is filtered.
