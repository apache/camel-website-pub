...
| **serverMessageLoggingLevel** (advanced) | 
...
| **caSignatureAlgorithms** (security) | Set a comma separated list of CA signature algorithms accepted for host certificate verification. If not specified the default list from JSch will be used (matches OpenSSH 8.2 defaults). |  | String |
...
| **publicKeyAcceptedAlgorithms** (security) | Set a comma separated list of public key accepted algorithms. Some examples include: ssh-dss,ssh-rsa,ecdsa-sha2-nistp256,ecdsa-sha2-nistp384,ecdsa-sha2-nistp521. If not specified the default list from JSCH will be used. |  | String |
...
> **Note**
> When a certificate is loaded, Camel automatically adds the certificate’s key type to JSch’s `PubkeyAcceptedAlgorithms` if it is not already present. This ensures certificate authentication works even when the key type has been removed from JSch’s defaults due to SHA-1 deprecation (e.g., `ssh-rsa-cert-v01@openssh.com`). Both the legacy vendor-prefixed key type names (`<algorithm>-[cert-v01@openssh.com](mailto:cert-v01@openssh.com)`, used through [draft-03](https://datatracker.ietf.org/doc/html/draft-miller-ssh-cert-03)) and the standardized short names (`<algorithm>-cert`, introduced in [draft-04](https://datatracker.ietf.org/doc/html/draft-miller-ssh-cert-04)) are recognized. If you need to control the CA signature algorithms that JSch will accept, use the `caSignatureAlgorithms` parameter.
...
Note that the key types and algorithms your server supports may differ than the above example. You can use the following command to inspect your server’s configuration:

ssh -vvv <server\_address>

As of Camel 3.18.1, these values can also be set on SFTP endpoints by setting the corresponding URI parameters.

## More Information

For more information, you can look at the [FTP component](ftp-component.md).

## Spring Boot Auto-Configuration

When using sftp with Spring Boot make sure to use the following Maven dependency to have support for auto configuration:
