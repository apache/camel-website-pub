...
When using spring-ai-vector-store with Spring Boot make sure to use the following Maven dependency to have support for auto configuration:
...
| **camel.component.spring-ai-vector-store.operation** | The operation to perform on the vector store (ADD, DELETE, SIMILARITY\_SEARCH). | add | SpringAiVectorStoreOperation |
