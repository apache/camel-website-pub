...
The Debezium MongoDB Connector component supports 87 options, which are listed below.
...
| **customSanitizePattern** (mongodb) | Regular expression identifying configuration keys whose values should be masked. When set, this custom pattern replaces Debeziums default password masking pattern. | .\*secret$|.\*password$|.\*sasl\\.jaas\\.config$|.\*basic\\.auth\\.user\\.info|.\*registry\\.auth\\.client-secret | String |
...
| **camel.component.debezium-mongodb.custom-sanitize-pattern** | Regular expression identifying configuration keys whose values should be masked. When set, this custom pattern replaces Debeziums default password masking pattern. | \\.jaas.config$|.\*basic.auth.user.info|.\*registry.auth.client-secret | String |
