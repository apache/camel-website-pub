...
The Browse component provides a simple BrowsableEndpoint which can be useful for testing, visualization tools or debugging. The exchanges sent to the endpoint are all available to be browsed.
...
The Browse component supports the following options which are listed below.
...
| **browseLimit** (advanced) | Maximum number of messages to keep in memory available for browsing. Use 0 for unlimited. |  | int |
...
| **browseLimit** (advanced) | Maximum number of messages to keep in memory available for browsing. Use 0 for unlimited. |  | int |
| **filter** (advanced) | To use a predicate to filter whether to include the message for browsing. Return true to include, false to exclude. |  | Predicate |
