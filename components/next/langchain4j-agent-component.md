...
| **mcpServer** (advanced) | MCP server definitions in the form of mcpServer..=. Supported properties: transportType (stdio, http or streamableHttp, default: stdio), command (comma-separated, for stdio), url (for http/streamableHttp), environment.= (for stdio), timeout (in seconds, default: 60), logRequests, logResponses, oauthProfile (OAuth profile for HTTP auth, requires camel-oauth). This is a multi-value option with prefix: mcpServer. |  | Map |
...
The chat model that drives the agent can be declared by its provider and the options every provider has, without a bean of the provider’s model class (since Camel 4.23). In `application.properties`, for every endpoint of the component:
...
The agent is then created as with an `AgentConfiguration` holding only that model (no memory); endpoints with the same provider options share one model instance. The provider is ignored when a model bean is configured (`agent`, `agentFactory` or `agentConfiguration`).
