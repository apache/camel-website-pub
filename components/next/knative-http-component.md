...
This is a good option when you have multiple options instances available in the Camel context. Read more about property binding configuration in the chapter [Property Binding](../../manual/property-binding.md).
...
-   camel.knative.client.ssl.<property\_name>
    
-   CAMEL\_KNATIVE\_CLIENT\_SSL\_<PROPERTY\_NAME>
...
-   camel.knative.client.oidc.<property\_name>
    
-   CAMEL\_KNATIVE\_CLIENT\_OIDC\_<PROPERTY\_NAME>
...
In order to refresh the token the Camel Knative client must read the token again. The Knative client options supports the token renewal on a `401 forbidden` response from the Knative broker.
