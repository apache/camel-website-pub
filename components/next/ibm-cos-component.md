...
When using ibm-cos with Spring Boot make sure to use the following Maven dependency to have support for auto configuration:
...
| **camel.component.ibm-cos.endpoint-url** | IBM COS Endpoint URL (e.g., [https://s3.us-south.cloud-object-storage.appdomain.cloud](https://s3.us-south.cloud-object-storage.appdomain.cloud)) Note that some operations requires to use a regional endpoint URL instead of a cross-region one. |  | String |
