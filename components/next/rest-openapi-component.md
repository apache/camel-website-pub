...
If you do not specify the _componentName_ at either component or endpoint level, a suitable delegate is looked up: a component already in the Camel context that implements `RestProducerFactory`, then the registry, then the default components in this order: `http`, `vertx-http`, `undertow`, `netty-http`. So with both camel-http and camel-vertx-http on the classpath, camel-http is used unless you name the other. The Camel CLI (`camel run`) adds camel-http for a rest-openapi producer.
