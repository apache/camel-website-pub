...
If the message body is stream based, then logging the message body may cause the message body to be empty afterward. For streamed messages, you can use [Stream caching](../../../manual/stream-caching.md) to allow logging the message body and be able to read the message body afterward again.
...
> **Tip**
> If you enable `sourceLocationEnabled=true` on `CamelContext` then Camel will use source _sourceFileName:lineNumber_ as logger name, instead of the route id. This is for example what `camel-jbang` do, to make it easy to see where in the source code the log is located.
...
In addition to name-based masking, `DefaultMaskingFormatter` also masks two value shapes that have no sensitive key name: URI userinfo passwords (`scheme://user:password@host`) and PEM private-key blocks (`-----BEGIN … PRIVATE KEY-----`). Certificates and public keys are left unchanged.
