...
The route and the code comments below explain how you can use the [Split](split-eip.md) EIP to split each message to sub-message which are processed individually and then combined back into a single response message via the custom `AggregationStrategy` (`myStrategy`), as the output from the Split EIP.
