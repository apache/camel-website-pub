...
The Mail component supports the following options which are listed below.
...
| **closeFolder** (consumer) | Whether the consumer should close the folder after polling. Setting this option to false and having disconnect=false as well, then the consumer keeps the folder open between polls. | true | boolean |
...
| **generateMissingAttachmentNames** (consumer (advanced)) | Set this to 'uuid' to set a UUID for the filename of the attachment if no filename was set. |  | String |
| **handleDuplicateAttachmentNames** (consumer (advanced)) | Set the strategy to handle duplicate filenames of attachments never: attachments that have a filename which is already present in the attachments will be ignored unless failOnDuplicateFileAttachment is set to true. uuidPrefix: this will prefix the duplicate attachment filenames each with an uuid and underscore (uuid\_filename.fileextension). uuidSuffix: this will suffix the duplicate attachment filenames each with an underscore and uuid (filename\_uuid.fileextension). |  | String |
...
| **authenticator** (advanced) | The authenticator for login. If set then the password and username are ignored. It can be used for tokens which can expire and therefore must be read dynamically. |  | MailAuthenticator |
...
| **maxMultipartDepth** (advanced) | The maximum nesting depth of multipart MIME parts the consumer descends into when extracting attachments. A message nested deeper than this has its deeper parts skipped, so a crafted deeply nested multipart cannot exhaust the stack. Increase it only if a legitimate source produces unusually deep nesting. | 20 | int |
...
| **searchTerm** (filter) | Refers to a jakarta.mail.search.SearchTerm which allows to filter mails based on search criteria such as subject, body, from, sent after a certain date etc. This is a multi-value option with prefix: searchTerm. |  | SearchTerm |
...
The Mail component supports the following message header(s), which is/are listed below:

   
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| **Subject** (common) Constant: [`MAIL_SUBJECT`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_SUBJECT) | Subject. |  | String |
| **From** (common) Constant: [`MAIL_FROM`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_FROM) | From. |  | String |
| **To** (common) Constant: [`MAIL_TO`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_TO) | To. |  | String |
| **Cc** (common) Constant: [`MAIL_CC`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_CC) | Cc. |  | String |
| **Bcc** (common) Constant: [`MAIL_BCC`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_BCC) | Bcc. |  | String |
| **Reply-To** (common) Constant: [`MAIL_REPLY_TO`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_REPLY_TO) | Reply to. |  | String |
| **contentType** (common) Constant: [`MAIL_CONTENT_TYPE`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_CONTENT_TYPE) | The content type. |  | String |
| **CamelMailCopyTo** (consumer) Constant: [`MAIL_COPY_TO`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_COPY_TO) | After processing a mail message, it can be copied to a mail folder with the given name. |  | String |
| **CamelMailMoveTo** (consumer) Constant: [`MAIL_MOVE_TO`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_MOVE_TO) | After processing a mail message, it can be moved to a mail folder with the given name. |  | String |
| **CamelMailDelete** (consumer) Constant: [`MAIL_DELETE`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_DELETE) | Deletes the messages after they have been processed. |  | boolean |
| **CamelMailMessageId** (common) Constant: [`MAIL_MESSAGE_ID`](https://javadoc.io/doc/org.apache.camel/camel-mail/latest/org/apache/camel/component/mail/MailConstants.html#MAIL_MESSAGE_ID) | The message ID. |  | String |
...
Camel uses the message exchange’s IN body as the [MimeMessage](http://java.sun.com/javaee/5/docs/api/javax/mail/internet/MimeMessage.md) text content. The body is converted to `String.class`.

Camel copies all of the exchange’s IN headers to the [MimeMessage](http://java.sun.com/javaee/5/docs/api/javax/mail/internet/MimeMessage.md) headers.

The subject of the [MimeMessage](http://java.sun.com/javaee/5/docs/api/javax/mail/internet/MimeMessage.md) can be configured using a header property on the IN message. The code below demonstrates this:
...
When using the MailProducer to send the mail to server, you should be able to get the message id of the [MimeMessage](http://java.sun.com/javaee/5/docs/api/javax/mail/internet/MimeMessage.md) with the key `CamelMailMessageId` from the Camel message header.
...
> **Note**
> These options do **not** strip the headers from the exchange. See the [Deployment hardening](../../manual/security-model.html#_deployment_hardening) section of the Security Model.
...
[JavaMail API](https://java.net/projects/javamail/pages/Home) is used under the hood for consuming and producing mails. We encourage end-users to consult these references when using either POP3 or IMAP protocol. Note particularly that POP3 has a much more limited set of features than IMAP.

-   [JavaMail POP3 API](https://javamail.java.net/nonav/docs/api/com/sun/mail/pop3/package-summary.md)
    
-   [JavaMail IMAP API](https://javamail.java.net/nonav/docs/api/com/sun/mail/imap/package-summary.md)
    
-   And generally about the [MAIL Flags](https://javamail.java.net/nonav/docs/api/javax/mail/Flags.md)
...
The code is provided out of the box in Camel 2.10 onwards in the `camel-mail` component. The code is in the class: `org.apache.camel.component.mail.SplitAttachmentsExpression`, which you can find the source code [here](https://github.com/apache/camel/blob/main/components/camel-mail/src/main/java/org/apache/camel/component/mail/SplitAttachmentsExpression.java)
...
When sending mails, then you can provide dynamic java mail properties for the `JavaMailSender` from the Exchange as message headers with keys starting with `mail.smtp.` (or `mail.smtps.`).
