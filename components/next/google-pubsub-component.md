...
Because the default is Application Default Credentials, a workload that already carries a Google identity needs no credential configuration at all: leave `serviceAccountKey` unset and ADC resolves the identity attached to the workload. On GKE with [Workload Identity](https://cloud.google.com/kubernetes-engine/docs/concepts/workload-identity) that is the GCP service account bound to the pod’s Kubernetes service account; the same applies on Compute Engine and Cloud Run.
