...
The Simple JMS component supports the following options which are listed below.
...
The Simple JMS component supports the following message header(s), which is/are listed below:
...
The SJMS component does not support for Spring or JTA Transaction, however, support for internal local transactions is supported using JMS or Transaction or Client Acknowledge Mode. See further details below.
...
SJMS producers that are `transacted=true` will also defer until the end of processing the message before the active JMS Session will commit or rollback.
...
The SJMS Component uses the same header format strategy used in the Camel JMS Component. This pluggable strategy ensures that messages sent over the wire conform to the JMS Message spec.
...
When using _InOut_ with SJMS in a clustered environment, you must either use TemporaryQueue destinations or use a unique reply to destination per InOut producer endpoint. The producer handles message correlation is handled, not with message selectors at the broker.
