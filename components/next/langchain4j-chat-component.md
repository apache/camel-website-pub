...
The LangChain4j Chat component supports the following options which are listed below.
...
The LangChain4j Chat component supports the following message header(s), which is/are listed below:
...
The same properties are set by [LangChain4j Agent](langchain4j-agent-component.md) and [LangChain4j Embeddings](langchain4j-embeddings-component.md) on their failure paths. See [AI LLM integration guide](ai-llm-integration-guide.html#_structured_error_exchange_properties) for a cross-component overview.

> **Important**
> The mapping is performed by the LangChain4j model implementation, not by Camel. `OpenAiChatModel` and `OllamaChatModel` route their calls through LangChain4j’s exception mapper, but a model that does not will propagate the raw exception of its own HTTP client instead. Verify what your provider throws before relying on the type.
