...
The Java language supports the following options which are listed below.
...
You can turn off pre-compilation for the Java language and then Camel will recompile an external script resource for each message (an inline script is still only compiled once, on first use, as its text cannot change). You can externalize the code into a resource file, which will be reloaded on each message as shown:
