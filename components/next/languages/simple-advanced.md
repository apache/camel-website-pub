...
Inside the init block, it is also possible to define custom functions in the syntax `$name ~:= <statement>;` where you can then use simple language to declare the structure of the function. Then you can later use these custom functions in your simple language expressions.
...
You can now provide a result type to the [Simple](simple-language.md) expression, which means the result of the evaluation will be converted to the desired type. This is most usable to define types such as booleans, integers, etc.
...
With the [Camel CLI](../../../manual/camel-jbang.md) the function does not have to be Java: the same class can be a Groovy source file next to the route, and Camel CLI compiles it and honours `@BindToRegistry`.
...
The name returned by `getName()` is the function name in simple, the value of `@BindToRegistry` is only the bean id:
    ...
    from:
      ...
      steps:
        - setHeader:
            ...
            expression:
              constant:
                expression: "john.doe@example.com"
        - setBody:
            expression:
              simple:
...
Run both files together (no `--dep` is needed, Camel CLI adds `camel-groovy` when a `.groovy` file is given):

```bash
camel run route.camel.yaml MaskEmailFunction.groovy --dev
```

The function can also be a bean in the YAML file itself, created by a Groovy script, so the whole example is one file. A Groovy map with the two methods of `SimpleFunction` is coerced to the interface with `as SimpleFunction`:
...
In dev mode (`--dev`) an edit of the function, in the `.groovy` file or in the YAML bean, is picked up on the next message: the bean is created again on reload and the simple expression resolves the function again per evaluation when the `dev` profile is active.

> **Note**
> The `.groovy` file keeps working in a project created with `camel export`: the Groovy compiler in camel-groovy binds a `@BindToRegistry` class itself, so the function is available to Camel in camel-main, Spring Boot and Quarkus alike. The bean lives in Camel’s registry, not in the Spring or CDI container, so it is not a candidate for `@Autowired` or `@Inject`; for that, write the class in Java.
