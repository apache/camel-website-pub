...
And you can generate unique IDs with the [Camel UUID Generator](../../../manual/uuidgenerator.md) by the `uuid` function. The generator supports different kinds `default`, `classic`, `short`, `simple` and `random`, such as `${uuid(random)}`.
