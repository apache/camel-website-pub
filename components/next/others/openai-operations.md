...
A `List` body moderates every element in a single API call, and `CamelOpenAIModerationFlagged` is then `true` when at least one element was flagged.
...
> **Note**
> The `illicit` and `illicit/violent` categories are optional in the API model. OpenAI returns them, but an [OpenAI-compatible provider](openai-providers.md) may not, in which case they are absent from the category map. The score map always contains every category.
...
1.  the MIME type detected as described in [the component MIME type detection](../openai-component.md) — the `CamelOpenAIMediaType` header, then the content type headers set by the object storage components, then `Content-Type`, then `CamelFileContentType`, then the file name;
