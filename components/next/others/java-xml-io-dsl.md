...
If you are using Camel XML DSL then it is recommended using `xml-io-dsl` instead of `xml-jaxb-dsl`. You can use this in all of Camel’s runtime such as Spring Boot, Quarkus, Camel Main.
...
It is now possible with `xml-io-dsl` to declare some beans to be bound to [Camel Registry](../../../manual/registry.md) in similar way as with [YAML DSL](yaml-dsl.md). Beans may be declared in XML and have their properties (also nested) defined. For example:
...
A class that has no public no-arg constructor but a public static `builder()` or `newBuilder()` method (Lombok `@Builder`, Immutables, LangChain4j, the AWS SDK v2, protobuf, the JDK `HttpClient` and many others) needs no `builderClass` and `builderMethod`: Camel calls the `builder()` method, sets the properties on the returned builder, and creates the bean with its `build()` method (or, when there is no `build()`, its single public no-arg method that returns the type):

```xml
	<bean name="chatModel" type="dev.langchain4j.model.ollama.OllamaChatModel">
        <properties>
          <property key="baseUrl" value="{{ollama.base.url}}"/>
          <property key="modelName" value="{{ollama.model}}"/>
        </properties>
	</bean>
```

Set `builderMethod` when the builder has several methods that return the type, or the method has another name. A class that has a public no-arg constructor is created with the constructor as usual, even if it also has a `builder()` method.
...
The init and destroy methods are optional, so a bean does not have to have both, for example you may only have a destroy methods.
