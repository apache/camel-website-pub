...
| `camel.main.duration` | For how long to keep running before automatic terminating the JVM, as a duration with a time unit such as 15s, 2m or 1h (a plain number is seconds). An alias of durationMaxSeconds that accepts a unit. |  | String |
...
| `camel.main.modeline` | Whether to support JBang style //DEPS to specify additional dependencies when running Camel CLI | false | boolean |
...
| `camel.main.startupRecorderRuntimeEnabled` | To also instrument the running Camel application, and not only its startup, by emitting Java Flight Recorder events for every exchange, route and processor. This requires that camel-jfr is on the classpath, and to enable this option. The runtime events are only captured while a recording is active. As they are emitted per message they add overhead, so this is turned off by default. | false | boolean |
...
| `camel.main.yamlDslCompactNotationWarn` | Whether to log a WARN when YAML DSL routes use compact (shorthand) notation instead of the canonical (explicit/normalized) form. The canonical style is recommended as it is more tooling and AI friendly. Use Camel CLI to normalize existing routes: camel validate normalize <file> | true | boolean |
...
| `camel.server.jwtAllowMissingIssuerAndAudience` | Whether JWT authentication is allowed to run without an expected issuer or audience. By default the embedded server fails to start when a JWT keystore is configured but neither jwtIssuer nor jwtAudience is set, because the tokens would only be checked for signature and expiry. Enable this to accept that and validate signature and expiry only. | false | boolean |
...
| `camel.server.mcpEnabled` | Whether to expose ai-tool routes as MCP tools and ai-resource routes as MCP resources. Requires camel-mcp-server on the classpath. Use setMcpTransport(String) to choose streamable HTTP (default) or stdio for IDE subprocess integration. By default, the MCP server is not enabled. | false | boolean |
| `camel.server.mcpInstructions` | Top-level MCP instructions returned to clients on initialize. |  | String |
| `camel.server.mcpPath` | HTTP path where the MCP endpoint is served. | /mcp | String |
| `camel.server.mcpResourceTimeout` | Per-read MCP resource execution timeout in milliseconds. A read exceeding the timeout returns an error to the MCP client; the underlying route keeps running until it completes on its own. | 20000 | long |
| `camel.server.mcpServerDescription` | MCP server description advertised to clients in serverInfo.description . |  | String |
| `camel.server.mcpServerIcons` | JSON array of MCP server icons advertised to clients in serverInfo.icons . Each entry must include a src URL and may include mimeType , sizes and theme . |  | String |
| `camel.server.mcpServerName` | MCP server name advertised to clients. Defaults to the CamelContext name. |  | String |
| `camel.server.mcpServerTitle` | MCP server display title advertised to clients in serverInfo.title . |  | String |
| `camel.server.mcpServerWebsiteUrl` | MCP server website URL advertised to clients in serverInfo.websiteUrl . |  | String |
...
| `camel.server.mcpTags` | Comma-separated list of tag patterns selecting the ai-tool routes to expose as MCP tools and the ai-resource routes to expose as MCP resources. Matching is case-insensitive and supports exact match, wildcard prefix ( {code foo} ), and {code } to match all tags. Only routes registered under a matching tag are exposed; the untagged default pools are never exposed. When not set, nothing is exposed. |  | String |
...
| `camel.server.mcpTransport` | MCP transport for ai-tool routes: http (streamable HTTP on the embedded server, default) or stdio (JSON-RPC on process stdin/stdout for local agent subprocesses). | http | String |
...
| `camel.management.jwtAllowMissingIssuerAndAudience` | Whether JWT authentication is allowed to run without an expected issuer or audience. By default the management server fails to start when a JWT keystore is configured but neither jwtIssuer nor jwtAudience is set, because the tokens would only be checked for signature and expiry. Enable this to accept that and validate signature and expiry only. | false | boolean |
...
| `camel.management.mcpEnabled` | Whether to expose dev/diagnostics MCP tools on this management server (requires camel-mcp-server on the classpath). Currently honored only when Camel JBang registers JbangDevMcpMainListener (for example camel run --mcp). For route-based MCP on the main HTTP server in plain camel-main, configure camel.server.mcpEnabled instead. Not intended for production use. | false | boolean |
| `camel.management.mcpPath` | HTTP path where the management MCP endpoint is served. | /mcp | String |
...
| `camel.management.openapiUiEnabled` | Whether to enable Swagger UI for REST DSL OpenAPI documents. If enabled then you can browse the API at context-path: /q/openapi. The OpenAPI document is expected at openapiUiSpecPath (by default /q/openapi.json from REST DSL apiContextPath ). | false | boolean |
| `camel.management.openapiUiSpecPath` | Path or URL of the OpenAPI document loaded by Swagger UI at /q/openapi. | /q/openapi.json | String |
...
The camel.trace supports 16 options, which are listed below.

   
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| `camel.trace.activityEnabled` | Whether activity tracking is enabled. When enabled, the backlog tracer will capture activity data that can be enriched with additional details from tracing (such as span decorator attributes). | false | boolean |
| `camel.trace.activitySize` | Defines how many completed exchange summaries to keep in the activity queue (should be between 1 - 1000). The activity queue captures lightweight metadata (no body or headers) each time an exchange completes a route, providing a rolling window of recent exchange activity. | 100 | int |
...
| `camel.ssl.selfSignedKeyType` | The key algorithm to use when generating the self-signed certificate for development use (requires selfSigned to be enabled). Supported values are EC (default, NIST P-256) and RSA (2048-bit). | EC | String |
...
| `camel.metrics.clearOnReload` | Clear the captured metrics data when Camel is reloading routes such as when using Camel CLI. | true | boolean |
...
| `camel.faulttolerance.delay` | Control how long the circuit breaker stays open. The default is 5 seconds. | 5000 | Long |
...
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| `camel.resilience4j.asynchronous` | Whether to use asynchronous (non-blocking) processing using Resilience4j’s CompletionStage-based decorators. When enabled, the caller thread is released immediately and processing completes asynchronously. | false | Boolean |
| `camel.resilience4j.automaticTransitionFromOpenToHalfOpenEnabled` | Enables automatic transition from OPEN to HALF\_OPEN state once the waitDurationInOpenState has passed. | false | Boolean |
| `camel.resilience4j.bulkheadEnabled` | Whether bulkhead is enabled or not on the circuit breaker. | false | Boolean |
| `camel.resilience4j.bulkheadFairCallHandlingEnabled` | Configures whether the bulkhead uses a fair calling strategy. When enabled (default), a fair strategy guarantees the order of incoming requests (FIFO). When disabled, no ordering is guaranteed and may improve throughput. | true | Boolean |
...
| `camel.resilience4j.bulkheadMaxWaitDuration` | Configures a maximum amount of time which the calling thread will wait to enter the bulkhead. Accepts Camel duration expressions (e.g. 500ms, 1s) or millisecond values. Default is 0 (no waiting). |  | String |
...
| `camel.resilience4j.slidingWindowSize` | Configures the size of the sliding window which is used to record the outcome of calls when the CircuitBreaker is closed. slidingWindowSize configures the size of the sliding window. Sliding window can either be count-based or time-based. If slidingWindowType is COUNT\_BASED, the last slidingWindowSize calls are recorded and aggregated. If slidingWindowType is TIME\_BASED, the calls of the last slidingWindowSize seconds are recorded and aggregated. The slidingWindowSize must be greater than 0. The minimumNumberOfCalls must be greater than 0. If the slidingWindowType is COUNT\_BASED, the minimumNumberOfCalls cannot be greater than slidingWindowSize . If the slidingWindowType is TIME\_BASED, you can pick whatever you want. Default slidingWindowSize is 100. | 100 | Integer |
| `camel.resilience4j.slidingWindowSynchronizationStrategy` | Configures the synchronization strategy for the sliding window. LOCK\_FREE uses a CAS-based lock-free algorithm for better performance under high concurrency. SYNCHRONIZED uses blocking locks with lower memory allocation. | SYNCHRONIZED | String |
| `camel.resilience4j.slidingWindowType` | Configures the type of the sliding window which is used to record the outcome of calls when the CircuitBreaker is closed. Sliding window can either be count-based or time-based. If slidingWindowType is COUNT\_BASED, the last slidingWindowSize calls are recorded and aggregated. If slidingWindowType is TIME\_BASED, the calls of the last slidingWindowSize seconds are recorded and aggregated. Default slidingWindowType is COUNT\_BASED. | COUNT\_BASED | String |
| `camel.resilience4j.slowCallDurationThreshold` | Configures the duration threshold above which calls are considered as slow and increase the slow calls percentage. Accepts Camel duration expressions (e.g. 60s, 1m) or millisecond values. Default value is 60 seconds. | 60s | String |
...
| `camel.resilience4j.timeoutDuration` | Configures the thread execution timeout. Accepts Camel duration expressions (e.g. 1s, 500ms) or millisecond values. Default value is 1000 millis (1 second). | 1000 | String |
...
| `camel.resilience4j.waitDurationInOpenState` | Configures the wait duration which specifies how long the CircuitBreaker should stay open, before it switches to half open. Accepts Camel duration expressions (e.g. 60s, 1m) or millisecond values. Default value is 60 seconds. | 60s | String |
...
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| `camel.cluster.file.acquireLockDelay` | The time to wait before starting to try to acquire the cluster lock. Note that if FileLockClusterService determines no cluster members are running or cannot reliably determine the cluster state, the initial delay is computed from the acquireLockInterval. | 1 | long |
| `camel.cluster.file.acquireLockDelayUnit` | The time unit for the acquireLockDelay | SECONDS | TimeUnit |
| `camel.cluster.file.acquireLockInterval` | The time to wait between attempts to try to acquire the cluster lock evaluated using wall-clock time. All cluster members must use the same value so leadership checks and leader liveness detection remain consistent. | 10 | long |
| `camel.cluster.file.acquireLockIntervalUnit` | The time unit for the acquireLockInterval | SECONDS | TimeUnit |
| `camel.cluster.file.clusterDataTaskMaxAttempts` | Sets how many times a cluster data task will run, counting both the first execution and subsequent retries in case of failure or timeout. The default is 5 attempts. This can be useful when the cluster data root is on network based file storage, where I/O operations may occasionally block for long or unpredictable periods. | 5 | int |
| `camel.cluster.file.clusterDataTaskTimeout` | Sets the timeout for a cluster data task (reading or writing cluster data). The default is 10 seconds. Timeouts are useful when the cluster data root is on network storage, where I/O operations may occasionally block for long or unpredictable periods. | 10 | long |
| `camel.cluster.file.clusterDataTaskTimeoutUnit` | The time unit for the clusterDataTaskTimeoutUnit | SECONDS | TimeUnit |
| `camel.cluster.file.enabled` | Whether to enable this cluster service. | true | boolean |
| `camel.cluster.file.heartbeatTimeoutMultiplier` | Multiplier applied to the cluster leader acquireLockInterval to determine how long followers should wait before considering the leader stale. For example, if the leader updates its heartbeat every 2 seconds and the heartbeatTimeoutMultiplier is 3, followers will tolerate up to 2s \* 3 = 6s of silence before declaring the leader unavailable. | 5 | int |
| `camel.cluster.file.root` | The root file path |  | String |

### Camel Kubernetes Cluster Service configurations

The camel.cluster.kubernetes supports 15 options, which are listed below.

   
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| `camel.cluster.kubernetes.clusterLabels` | Labels used to identify the members of the cluster. |  | Map |
| `camel.cluster.kubernetes.configuration` | To use an existing configuration for Kubernetes |  | KubernetesConfiguration |
| `camel.cluster.kubernetes.connectionTimeout` | Connection timeout in milliseconds to use when making requests to the Kubernetes API server. |  | Integer |
| `camel.cluster.kubernetes.enabled` | Whether to enable this cluster service. | true | boolean |
| `camel.cluster.kubernetes.groupName` | Name of the lock group (or namespace according to the Camel cluster convention) within the chosen ConfigMap. |  | String |
| `camel.cluster.kubernetes.jitterFactor` | A jitter factor to apply in order to prevent all pods to call Kubernetes APIs in the same instant. | 1.2 | double |
| `camel.cluster.kubernetes.kubernetesResourceName` | Name of the resource used for locking (or prefix, in case multiple ones are used). | leaders | String |
| `camel.cluster.kubernetes.kubernetesResourcesNamespace` | Kubernetes namespace containing the pods and the ConfigMap used for locking. |  | String |
| `camel.cluster.kubernetes.leaseDurationMillis` | The default duration of the lease for the current leader. | 15000 | long |
| `camel.cluster.kubernetes.leaseResourceType` | Kubernetes resource type used to hold the leases. | Lease | LeaseResourceType |
| `camel.cluster.kubernetes.masterUrl` | URL to a remote Kubernetes API server. This should only be used when your Camel application is connecting from outside Kubernetes. If you run your Camel application inside Kubernetes, then you can use local or client as the URL to tell Camel to run in local mode. If you connect remotely to Kubernetes, then you may also need some of the many other configuration options for secured connection with certificates, etc. |  | String |
| `camel.cluster.kubernetes.namespace` | Kubernetes namespace |  | String |
| `camel.cluster.kubernetes.podName` | Name of the current pod (defaults to host name). |  | String |
| `camel.cluster.kubernetes.renewDeadlineMillis` | The deadline after which the leader must stop its services because it may have lost the leadership. | 10000 | long |
| `camel.cluster.kubernetes.retryPeriodMillis` | The time between two subsequent attempts to check and acquire the leadership. It is randomized using the jitter factor. | 2000 | long |

### Camel ZooKeeper Cluster Service configurations

The camel.cluster.zookeeper supports 12 options, which are listed below.

   
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| `camel.cluster.zookeeper.basePath` | The base path to store in ZooKeeper |  | String |
| `camel.cluster.zookeeper.connectTimeout` | Connect timeout (millis) | 15000 | long |
| `camel.cluster.zookeeper.enabled` | Whether to enable this cluster service. | true | boolean |
| `camel.cluster.zookeeper.id` | Node id |  | String |
| `camel.cluster.zookeeper.maxCloseWait` | Time to wait (millis) during close to join background threads | 1000 | long |
...
| `camel.cluster.zookeeper.reconnectBaseSleepTime` | Initial amount of time (millis) to wait between retries | 1000 | long |
| `camel.cluster.zookeeper.reconnectMaxRetries` | Max number of times to retry | 3 | int |
| `camel.cluster.zookeeper.reconnectMaxSleepTime` | Max time (millis) to sleep on each retry |  | long |
| `camel.cluster.zookeeper.retryPolicy` | To use a custom retry policy implementation |  | RetryPolicy |
| `camel.cluster.zookeeper.sessionTimeout` | Session timeout (millis) | 60000 | long |

### Camel Consul Cluster Service configurations

The camel.cluster.consul supports 17 options, which are listed below.

   
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| `camel.cluster.consul.aclToken` | Sets the ACL token to be used with Consul |  | String |
| `camel.cluster.consul.blockSeconds` | The second to wait for a watch event, default 10 seconds | 10 | int |
| `camel.cluster.consul.configuration` | To use an existing configuration |  | ConsulClusterConfiguration |
| `camel.cluster.consul.connectTimeout` | Connect timeout in millis |  | int |
| `camel.cluster.consul.datacenter` | The data center |  | String |
| `camel.cluster.consul.enabled` | Whether to enable this cluster service. | true | boolean |
| `camel.cluster.consul.id` | Client id registered as \_consul.service.registry.id |  | String |
| `camel.cluster.consul.password` | Sets the password to be used for basic authentication |  | String |
| `camel.cluster.consul.readTimeout` | Read timeout in millis |  | int |
| `camel.cluster.consul.rootPath` | The Consul cluster root directory path | /camel | String |
| `camel.cluster.consul.sessionLockDelay` | Session lock delay in seconds | 5 | int |
| `camel.cluster.consul.sessionRefreshInterval` | Session refresh interval in seconds | 5 | int |
| `camel.cluster.consul.sessionTtl` | Session time to live in seconds | 60 | int |
| `camel.cluster.consul.sslContextParameters` | SSL configuration for advanced security configuration |  | SSLContextParameters |
| `camel.cluster.consul.url` | The Consul agent URL |  | String |
| `camel.cluster.consul.userName` | Sets the username to be used for basic authentication |  | String |
| `camel.cluster.consul.writeTimeout` | Write timeout in mills |  | int |

### Camel JGroups Raft Cluster Service configurations

The camel.cluster.jgroups-raft supports 5 options, which are listed below.

   
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| `camel.cluster.jgroups-raft.enabled` | Whether to enable this cluster service. | true | boolean |
| `camel.cluster.jgroups-raft.id` | Unique Raft id |  | String |
| `camel.cluster.jgroups-raft.jgroupsClusterName` | The name of the cluster | jgroupsraft-master | String |
| `camel.cluster.jgroups-raft.jgroupsConfig` | The path to the JGroups Raft configuration | raft.xml | String |
| `camel.cluster.jgroups-raft.raftHandle` | To use a custom RaftHandler |  | RaftHandle |

### Camel Infinispan Remote Cluster Service configurations

The camel.cluster.infinispan supports 16 options, which are listed below.

   
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| `camel.cluster.infinispan.cacheContainer` | Specifies the cache Container to connect |  | RemoteCacheManager |
| `camel.cluster.infinispan.cacheContainerConfiguration` | The CacheContainer configuration. Used if the cacheContainer is not defined. |  | Configuration |
| `camel.cluster.infinispan.configuration` | To use an existing configuration |  | InfinispanRemoteClusterConfiguration |
| `camel.cluster.infinispan.configurationProperties` | Implementation specific properties for the CacheManager |  | Map |
| `camel.cluster.infinispan.configurationUri` | An implementation specific URI for the CacheManager |  | String |
| `camel.cluster.infinispan.enabled` | Whether to enable this cluster service. | true | boolean |
| `camel.cluster.infinispan.hosts` | Specifies the host of the cache on Infinispan instance. Multiple hosts can be separated by semicolon. |  | String |
| `camel.cluster.infinispan.id` | Node id |  | String |
| `camel.cluster.infinispan.lifespan` | The lifespan of the cache entry for the local cluster member registered to the inventory | 30 | long |
| `camel.cluster.infinispan.lifespanTimeUnit` | The TimeUnit of the lifespan | SECONDS | TimeUnit |
| `camel.cluster.infinispan.password` | Define the password to access the infinispan instance |  | String |
| `camel.cluster.infinispan.saslMechanism` | Define the SASL Mechanism to access the infinispan instance |  | String |
| `camel.cluster.infinispan.secure` | Define if we are connecting to a secured Infinispan instance | false | boolean |
| `camel.cluster.infinispan.securityRealm` | Define the security realm to access the infinispan instance |  | String |
| `camel.cluster.infinispan.securityServerName` | Define the security server name to access the infinispan instance |  | String |
| `camel.cluster.infinispan.username` | Define the username to access the infinispan instance |  | String |
...
### Creating custom beans from a builder

A class that has no public no-arg constructor but a public static `builder()` or `newBuilder()` method (Lombok `@Builder`, Immutables, LangChain4j, Spring AI, the AWS SDK v2, Jackson, protobuf, the JDK `HttpClient` and many others) is created via its builder: Camel calls the `builder()` method, sets the properties on the returned builder, and creates the bean with its `build()` method. A LangChain4j chat model is then declared like any other bean, with the property names of the builder’s fluent setters:
...
camel.beans.chatModel.timeout = 2m
```

If the builder has no `build()` method, Camel uses its single public no-arg method that returns the type. A class that has a public no-arg constructor is created with the constructor as usual, even if it also has a `builder()` method; a class whose builder is a separate class, or has several methods that return the type, is declared in YAML or XML with `builderClass` and `builderMethod` (see the [YAML DSL](../../4.22.x/others/yaml-dsl.md)).
