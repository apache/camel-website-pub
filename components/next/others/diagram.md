...
When the developer console is enabled (for example with `camel run --console`), the `route-diagram` console renders the running routes using the interactive `<camel-route-diagram>` web component, fed by the `route-structure` console. Request `?format=png` to get the legacy inline PNG image instead, or use the `ascii` / `unicode` themes for plain-text output.
...
`camel-diagram` ships a lightweight `<camel-route-diagram>` web component that renders interactive route diagrams as SVG directly in the browser. Any application with `camel-diagram` on the classpath automatically serves the component as a static resource — no extra server configuration needed.
...
The component is theme-agnostic. It respects `prefers-color-scheme` automatically for dark/light mode, and exposes CSS custom properties so the host application can override every visual aspect:
