...
The JSONPath language supports 10 options, which are listed below.
...
| **unpackArray** | `false` | `Boolean` | Whether to unpack a single element json-array into an object. |
| **option** |  | `Enum` | 
...
| **camel.language.jsonpath.header-name** | Name of header to use as input, instead of the message body It has as higher precedent than the propertyName if both are set. |  | String |
...
| **camel.language.jsonpath.property-name** | Name of property to use as input, instead of the message body. It has a lower precedent than the headerName if both are set. |  | String |
