...
| **propertyName** (common) | To use a Camel Exchange property as the input source instead of Message body. It has a lower precedent than the name of header if both are set. |  | String |
...
You can also use functions inside your query, in which case you need an explicit type conversion (or you will get a org.w3c.dom.DOMException: HIERARCHY\_REQUEST\_ERR) by passing the Class as a second argument to the **xquery()** method.
