...
The Netty HTTP component is an extension to [Netty](netty-component.md) component to facilitiate HTTP transport with [Netty](netty-component.md).
...
> **A lot more options**
>
> This component inherits all the options from [Netty](netty-component.md), so make sure to look at the [Netty](netty-component.md) documentation as well. Notice that some options from [Netty](netty-component.md) is not applicable when using this Netty HTTP component, such as options related to UDP transport.
...
The Netty HTTP component supports 80 options, which are listed below.
...
The Netty HTTP component supports 23 message header(s), which is/are listed below:
...
| **camel.component.netty-http.producer-pool-max-total** | Sets the cap on the number of objects that can be allocated by the pool (checked out to clients, or idle awaiting checkout) at a given time. Use a negative value for no limit. | \-1 | Integer |
