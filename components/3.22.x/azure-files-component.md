...
> **Caution**
> A preview component so anything can change in a next release or it could be even dropped. At the same time it is consolidated enough, sparingly documented, a few users reported it was working in their environment, and it is ready for wider feedback.
...
The `autoCreate` option is supported for the directory, when consumer or producer starts, there’s an additional operation performed to create the directory configured for the endpoint. The default value for `autoCreate` is `true`. On contrary, the share must exist, it is not automatically created.
...
The SAS token or shared key based auth is supported, the options for selected one must be present.
...
The Azure Files component supports 3 options, which are listed below.
...
| **sharedKey** (both) | Shared key (storage account key). |  | String |
...
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| **CamelFileLength** (both) Constant: [`FILE_LENGTH`](https://javadoc.io/doc/org.apache.camel/camel-azure-files/latest/org/apache/camel/component/file/azure/FilesHeaders.html#FILE_LENGTH) | A long value containing the file size. For producer, known length helps if the body converts to InputStream more efficiently than to bytes array. |  | long |
| **CamelFileLastModified** (consumer) Constant: [`FILE_LAST_MODIFIED`](https://javadoc.io/doc/org.apache.camel/camel-azure-files/latest/org/apache/camel/component/file/azure/FilesHeaders.html#FILE_LAST_MODIFIED) | A Long value containing the last modified timestamp of the file. |  | long |
| **CamelFileName** (common) Constant: [`FILE_NAME`](https://javadoc.io/doc/org.apache.camel/camel-azure-files/latest/org/apache/camel/component/file/azure/FilesHeaders.html#FILE_NAME) | Specifies the output file name (relative to the endpoint directory) to be used for the output message when sending to the endpoint. If this is not present and no expression either, then a generated message ID is used as the filename instead. |  | String |
| **CamelFileNameOnly** (common) Constant: [`FILE_NAME_ONLY`](https://javadoc.io/doc/org.apache.camel/camel-azure-files/latest/org/apache/camel/component/file/azure/FilesHeaders.html#FILE_NAME_ONLY) | Only the file name (the name with no leading paths). |  | String |
| **CamelFileParent** (common) Constant: [`FILE_PARENT`](https://javadoc.io/doc/org.apache.camel/camel-azure-files/latest/org/apache/camel/component/file/azure/FilesHeaders.html#FILE_PARENT) | The parent path. |  | String |
| **CamelRemoteFileInputStream** (common) Constant: [`REMOTE_FILE_INPUT_STREAM`](https://javadoc.io/doc/org.apache.camel/camel-azure-files/latest/org/apache/camel/component/file/azure/FilesHeaders.html#REMOTE_FILE_INPUT_STREAM) | The remote file input stream. |  | InputStream |
| **CamelFileLocalWorkPath** (common) Constant: [`FILE_LOCAL_WORK_PATH`](https://javadoc.io/doc/org.apache.camel/camel-azure-files/latest/org/apache/camel/component/file/azure/FilesHeaders.html#FILE_LOCAL_WORK_PATH) | Path to the local work file, if local work directory is used. |  | String |
| **CamelFileHost** (common) Constant: [`FILE_HOST`](https://javadoc.io/doc/org.apache.camel/camel-azure-files/latest/org/apache/camel/component/file/azure/FilesHeaders.html#FILE_HOST) | The remote hostname. |  | String |
...
When moving files using `move` or `preMove` option, the files are restricted to the share. That prevents consumer from moving files outside of the endpoint share.
...
The underlying Azure Files service does not allow to grow files. The file length must be known at its creation time, consequently:
...
For now the files upload has no timeout. During the upload, the underlying library could log timeout warnings. They are recoverable and upload could continue.
...
to optimize _the single file using a fixed name_ use case, it is necessary to fallback to regular filters (i.e. the list permission is needed).
