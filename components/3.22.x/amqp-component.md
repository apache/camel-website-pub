...
You can also rely on the [Camel properties](properties-component.md) to read the AMQP connection details. Factory method `AMQPConnectionDetails.discoverAMQP()` attempts to read Camel properties in a Kubernetes-like convention, just as demonstrated on the snippet below:
...
| **camel.component.amqp.delivery-mode** | Specifies the delivery mode to be used. Possible values are those defined by javax.jms.DeliveryMode. NON\_PERSISTENT = 1 and PERSISTENT = 2. |  | Integer |
