...
[Azure Cosmos DB](https://azure.microsoft.com/en-us/services/cosmos-db/) is Microsoft’s globally distributed, multi-model database service for operational and analytics workloads. It offers multi-mastering feature by automatically scaling throughput, compute, and storage. This component interacts with Azure CosmosDB through Azure SQL API.
...
The Azure CosmosDB component supports 30 options, which are listed below.
...
| **useDefaultIdentity** (security) | Indicates whether to use the default identity mechanism instead of the access key. | false | boolean |
...
| **camel.component.azure-cosmosdb.use-default-identity** | Indicates whether to use the default identity mechanism instead of the access key. | false | Boolean |