...
The following code will register an EventListener under the path import-application/inbox for Event.NODE\_ADDED and Event.NODE\_REMOVED events (event types 1 and 2, both masked as 3) and listening deep for all the children.
...
| Name | Description | Default | Type |
| --- | --- | --- | --- |
| **camel.component.jcr.autowired-enabled** | Whether autowiring is enabled. This is used for automatic autowiring options (the option must be marked as autowired) by looking up in the registry to find if there is a single instance of matching type, which then gets configured on the component. This can be used for automatic configuring JDBC data sources, JMS connection factories, AWS Clients, etc. | true | Boolean |
