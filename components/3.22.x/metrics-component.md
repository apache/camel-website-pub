...
Each metric has type and name. Supported types are [counter](#MetricsComponent-counter), [histogram](#MetricsComponent-histogram), [meter](#MetricsComponent-meter), [timer](#MetricsComponent-timer) and [gauge](#MetricsComponent-gauge). Metric name is simple string. If metric type is not provided then type meter is used by default.
