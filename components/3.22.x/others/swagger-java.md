...
> **Warning**
> **Deprecated:** This swagger-java is deprecated and may be removed in a future release.

**Since Camel 2.16**

The Rest DSL can be integrated with the `camel-swagger-java` module which is used for exposing the REST services and their APIs using [Swagger](http://swagger.io/).
...
| base.path | String | **Required**: To setup the base path where the REST services is available. The path is relative (eg do not start with http/https) and camel-swagger-java will calculate the absolute base path at runtime, which will be `protocol://host:port/context-path/base.path` |
| api.path | String | To setup the path where the API is available (eg /api-docs). The path is relative (eg do not start with http/https) and camel-swagger-java will calculate the absolute base path at runtime, which will be `protocol://host:port/context-path/api.path` So using relative paths is much easier. See above for an example. |
...
The Rest DSL now supports declaring Swagger `securityDefinitions` in the generated API document. For example as shown below:
...
The Swagger specification allows you to specify the host, port & path that is serving the API. This is done via the `host` field.
...
When using swagger-java with Spring Boot make sure to use the following Maven dependency to have support for auto configuration:
