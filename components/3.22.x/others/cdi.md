...
[Producer](http://docs.jboss.org/cdi/spec/2.0/cdi-spec.html#producer_method) and [disposer](http://docs.jboss.org/cdi/spec/2.0/cdi-spec.html#disposer_method) methods can also be used as well to customize the Camel context bean, e.g.:
...
Similarly, [producer fields](http://docs.jboss.org/cdi/spec/2.0/cdi-spec.html#producer_field) can be used, e.g.:
...
If you want to use [DeltaSpike configuration mechanism](http://deltaspike.apache.org/documentation/configuration.md) you can declare the following `PropertiesComponent` bean:
...
As part of the Camel [bean integration](http://camel.apache.org/bean-integration.md), Camel comes with a set of [annotations](http://camel.apache.org/bean-integration.html#BeanIntegration-Annotations) that are seamlessly supported by Camel CDI. So you can use any of these annotations in your CDI beans, e.g.:
...
```
 ...
 | If using [DeltaSpike configuration mechanism](http://deltaspike.apache.org/documentation/configuration.md):
...
Camel provides a set of [events](https://www.javadoc.io/doc/org.apache.camel/camel-api/latest/org/apache/camel/spi/CamelEvent.md) that can be subscribed to for listening to Camel context, service, route and exchange events. Camel CDI seamlessly translates these Camel events into CDI events that can be observed using CDI [observer methods](https://docs.jboss.org/cdi/spec/2.0/cdi-spec.html#observer_methods), e.g.:
...
The CDI event endpoint bridges the [CDI events](http://docs.jboss.org/cdi/spec/2.0/cdi-spec.html#events) with the Camel routes so that CDI events can be seamlessly observed / consumed (resp. produced / fired) from Camel consumers (resp. by Camel producers).

The `CdiEventEndpoint<T>` bean provided by Camel CDI can be used to observe / consume CDI events whose _event type_ is `T`, for example:
...
The type variable `T` (resp. the qualifiers) of a particular `CdiEventEndpoint<T>` injection point are automatically translated into the parameterized _event type_ (resp. into the _event qualifiers_) e.g.:
...
Note that the CDI event Camel endpoint dynamically adds an [observer method](http://docs.jboss.org/cdi/spec/2.0/cdi-spec.html#observer_methods) for each unique combination of _event type_ and _event qualifiers_ and solely relies on the container typesafe [observer resolution](http://docs.jboss.org/cdi/spec/2.0/cdi-spec.html#observer_resolution), which leads to an implementation as efficient as possible.

Besides, as the impedance between the _typesafe_ nature of CDI and the _dynamic_ nature of the [Camel component](http://camel.apache.org/component.md) model is quite high, it is not possible to create an instance of the CDI event Camel endpoint via [URIs](http://camel.apache.org/uris.md). Indeed, the URI format for the CDI event component is:

```text
cdi-event://PayloadType<T1,...,Tn>[?qualifiers=QualifierType1[,...[,QualifierTypeN]...]]
```

With the authority `PayloadType` (resp. the `QualifierType`) being the URI escaped fully qualified name of the payload (resp. qualifier) raw type followed by the type parameters section delimited by angle brackets for payload parameterized type. Which leads to _unfriendly_ URIs, e.g.:
...
While the CDI programmatic model favors a [typesafe resolution](http://docs.jboss.org/cdi/spec/2.0/cdi-spec.html#typesafe_resolution) mechanism that occurs at application initialization time, it is possible to perform dynamic / lazy injection later during the application execution using the [programmatic lookup](http://docs.jboss.org/cdi/spec/2.0/cdi-spec.html#programmatic_lookup) mechanism.
...
Among the available [Camel Maven archetypes](../../../manual/camel-maven-archetypes.md), you can use the provided `camel-archetype-cdi` to generate a Camel CDI Maven project, e.g.:
