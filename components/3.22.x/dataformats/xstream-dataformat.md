...
> **Warning**
> **Deprecated:** This xstream is deprecated and may be removed in a future release.

**Since Camel 1.3**

XStream is a Data Format which uses the [XStream library](http://x-stream.github.io/) to marshal and unmarshal Java objects to and from XML.
...
| **mode** |  | `Enum` | 
Mode for dealing with duplicate references The possible values are: NO\_REFERENCES ID\_REFERENCES XPATH\_RELATIVE\_REFERENCES XPATH\_ABSOLUTE\_REFERENCES SINGLE\_NODE\_XPATH\_RELATIVE\_REFERENCES SINGLE\_NODE\_XPATH\_ABSOLUTE\_REFERENCES.
...
-   XPATH\_RELATIVE\_REFERENCES
    
-   XPATH\_ABSOLUTE\_REFERENCES
    
-   SINGLE\_NODE\_XPATH\_RELATIVE\_REFERENCES
    
-   SINGLE\_NODE\_XPATH\_ABSOLUTE\_REFERENCES
...
| **converters** |  | `Array` | List of class names for using custom XStream converters. The classes must be of type com.thoughtworks.xstream.converters.Converter. |
| **aliases** |  | `Array` | Alias a Class to a shorter name to be used in XML elements. |
| **omitFields** |  | `Array` | Prevents a field from being serialized. To omit a field you must always provide the declaring type and not necessarily the type that is converted. Multiple values can be separated by comma. |
| **implicitCollections** |  | `Array` | Adds a default implicit collection which is used for any unmapped XML tag. Multiple values can be separated by comma. |
...
[The XStream library](http://x-stream.github.io/) uses the `javax.xml.stream.XMLInputFactory` and `javax.xml.stream.XMLOutputFactory`, you can control which implementation of this factory should be used.
...
In Camel, one can always use its own processing step in the route to filter and block certain XML documents to be routed to the XStream’s unmarhall step. You can set [XStream’s type permissions](http://x-stream.github.io/security.md) to automatically allow or deny the instantiation of certain types.
