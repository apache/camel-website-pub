...
Jackson XML is a Data Format which uses the [Jackson library](https://github.com/FasterXML/jackson/) with the [XMLMapper extension](https://github.com/FasterXML/jackson-dataformat-xml) to unmarshal an XML payload into Java objects or to marshal Java objects into an XML payload.
...
The Jackson XML dataformat supports 16 options, which are listed below.
...
If there is a single `XmlMapper` instance available in the Camel registry, it will used by the converter to perform the conversion. Otherwise the default mapper will be used.
