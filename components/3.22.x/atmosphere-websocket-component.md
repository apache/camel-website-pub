...
The component uses the [SERVLET](servlet-component.md) component and uses the [Atmosphere](https://github.com/Atmosphere/atmosphere) library to support the Websocket transport in various Servlet containers (e..g., Jetty, Tomcat, …​).

Unlike the [Websocket](websocket-component.md) component that starts the embedded Jetty server, this component uses the servlet provider of the container.
...
| **camel.component.atmosphere-websocket.mute-exception** | If enabled and an Exchange failed processing on the consumer side the response’s body won’t contain the exception’s stack trace. | false | Boolean |
