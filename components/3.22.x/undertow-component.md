...
| **sendToAll** (websocket) | To send to all websocket subscribers. Can be used to configure on endpoint level, instead of having to use the UndertowConstants.SEND\_TO\_ALL header on the message. |  | Boolean |
...
By default [Jetty](../4.18.x/jetty-component.md) will only match on exact uri’s. But you can instruct Jetty to match prefixes. For example:

```java
from("jetty://0.0.0.0:8123/foo").to("mock:foo");
```

In the route above [Jetty](../4.18.x/jetty-component.md) will only match if the uri is an exact match, so it will match if you enter `http://0.0.0.0:8123/foo` but not match if you do `http://0.0.0.0:8123/foo/bar`.
