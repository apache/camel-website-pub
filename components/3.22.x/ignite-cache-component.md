...
The Ignite Cache endpoint is one of camel-ignite endpoints which allows you to interact with an [Ignite Cache](https://apacheignite.readme.io/docs/data-grid). This offers both a Producer (to invoke cache operations on an Ignite cache) and a Consumer (to consume changes from a continuous query).
...
| **CamelIgniteCacheOldValue** (consumer) Constant: [`IGNITE_CACHE_OLD_VALUE`](https://javadoc.io/doc/org.apache.camel/camel-ignite/latest/org/apache/camel/component/ignite/IgniteConstants.html#IGNITE_CACHE_OLD_VALUE) | This header carries the old cache value when passed in the incoming cache event. |  | Object |
