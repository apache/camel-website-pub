...
In Apache Camel, you can implement the normalizer pattern by combining a [Content Based Router](choice-eip.md), which detects the incoming message’s format, with a collection of different [Message Translator](message-translator.md)'s, which transform the different incoming formats into a common format.
