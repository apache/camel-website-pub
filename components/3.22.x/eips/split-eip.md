...
The [Splitter](http://www.enterpriseintegrationpatterns.com/patterns/messaging/Sequencer.md) from the [EIP patterns](enterprise-integration-patterns.md) allows you split a message into a number of pieces and process them individually.

## Options

The Split eip supports 14 options, which are listed below.
...
| **streaming** | When in streaming mode, then the splitter splits the original message on-demand, and each split message is processed one by one. This reduces memory usage as the splitter do not split all the messages first, but then we do not know the total size, and therefore the org.apache.camel.Exchange#SPLIT\_SIZE is empty. In non-streaming mode (default) the splitter will split each message first, to know the total size, and then process each message one by one. This requires to keep all the split messages in memory and therefore requires more memory. The total size is provided in the org.apache.camel.Exchange#SPLIT\_SIZE header. The streaming mode also affects the aggregation behavior. If enabled then Camel will process replies out-of-order, e.g. in the order they come back. If disabled, Camel will process replies in the same order as the messages was split. | false | Boolean |
...
| `CamelSplitSize` | `` `int` `` | The total number of Exchanges that was split. This property is not applied for stream based splitting, except for the very last message because then Camel knows the total size. |
...
In the example above then sending to mock:result happens after the Spllit EIP has finished. In other words the message should finish splitting the entire message before the message continues.
...
The `group` value must be a positive number dictating how many elements to combine together in a group. Each part will be combined using the token.
...
The Splitter will by default continue to process the entire Exchange even in case of one of the split message will throw an exception during routing.
...
The error handler in this example is configured to retry up till 3 times. And when a split message fails all redelivery attempts (its exhausted), then this message is **not** moved into that dead letter queue.
...
Because [Multicast](multicast-eip.md) EIP is baseline for the [Recipient List](recipientList-eip.md) and Split EIPs, then you can find more information in those EIPs about features that is also available with Split EIP.