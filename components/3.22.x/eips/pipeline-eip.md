...
With Camel, you can separate your processing across multiple independent [Endpoint](../../../manual/endpoint.md)'s which can then be chained together.
...
Using pipeline becomes necessary when you need to group together a series of steps into a single logical step. For example in the example below where [Multicast](multicast-eip.md) EIP is in use, to process the same message in two different pipelines. The first pipeline calls the something bean, and the second pipeline calls the foo and bar beans and then route the message to another queue.
