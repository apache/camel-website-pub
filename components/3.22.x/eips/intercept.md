...
-   `interceptFrom` that intercepts only the incoming step (i.e. [from](from-eip.md))
    
-   `interceptSendToEndpoint` that intercepts only when an [Exchange](../../../manual/exchange.md) is about to be sent to the given [endpoint](message-endpoint.md).
...
All the interceptors can be configured on global, route scope, or with [Route Configuration](../../../manual/route-configuration.md).

This means multiple interceptors can be _triggered_.

Most of the examples in this page are on global scope. To use route scope, then it is similar, but it is done on the route as shown in the following example.
...
-   [Predicate](../../../manual/predicate.md) using `when` to only trigger the interceptor in certain conditions
...
-   `afterUri` when used with `interceptSendToEndpoint` allows to send the message to an [endpoint](message-endpoint.md) afterwards.
...
-   The intercepted endpoint uri is stored as message header with the key `Exchange.INTERCEPTED_ENDPOINT`.
    

### Using intercept

The `Intercept` is intercepting the [Exchange](../../../manual/exchange.md) on every processing steps during routing.
...
It is also possible to stop continue routing after being intercepted. Now suppose that if the message body contains the word Hello we want to log and stop, then we can do:
...
So lets start with the logging example. We want to log all the incoming messages, so we use `interceptFrom` to route to the [Log](../log-component.md) component.
...
You can also intercept when Camel is sending a message to an [endpoint](message-endpoint.md).
...
The `afterUri` option, is used when you need to process the response message from the intended destination. This functionality was added later to the interceptor, in a form of sending to yet another [endpoint](message-endpoint.md).
...
Match by wildcard allows you to match a range of endpoints or all of a given type. For instance use `file:*` will match all [file](../file-component.md) based endpoints.
