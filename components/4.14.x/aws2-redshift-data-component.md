...
The AWS RedshiftData component supports 22 options, which are listed below.
...
The AWS RedshiftData component supports 25 message header(s), which is/are listed below:
...
When using aws2-redshift-data with Spring Boot make sure to use the following Maven dependency to have support for auto configuration:
...
| **camel.component.aws2-redshift-data.region** | The region in which RedshiftData client needs to work. When using this parameter, the configuration will expect the lowercase name of the region (for example, ap-east-1) You’ll need to use the name Region.EU\_WEST\_1.id(). |  | String |
