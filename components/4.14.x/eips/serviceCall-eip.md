...
> **Warning**
> **Deprecated:** This serviceCall is deprecated and may be removed in a future release.

> **Important**
> The Service Call EIP is deprecated in Camel 3.x and will be removed in a future Camel release. There is no direct replacement. If you use Kubernetes, then services can be called directly by name.
...
Use a Service Call acting as a [Messaging Gateway](messaging-gateway.md) for distributed systems that handles the complexity of calling the service in a reliable manner.

The pattern has the following noteworthy features:

-   _Location transparency_: Decouples Camel and the physical location of the services using logical names representing the services.
    
-   _URI templating_: Allows you to template the Camel endpoint URI as the physical endpoint to use when calling the service.
    
-   _Service discovery_: it looks up the service from a service registry of some sort to know the physical locations of the services.
    
-   _Service filter_: Allows you to filter unwanted services (for example, blacklisted or unhealthy services).
    
-   _Service chooser_: Allows you to choose the most appropriate service based on factors such as geographical zone, affinity, plans, canary deployments, and SLAs.
    
-   _Load balancer_: A preconfigured Service Discovery, Filter, and Chooser intended for a specific runtime (these three features combined as one).
...
The Service Call eip supports 0 options, which are listed below.
...
| **expression** | Configures the Expression using the given configuration. |  | ServiceCallExpressionConfiguration |
...
The Service Call eip has no exchange properties.
...
When calling a service, you may refer to the name of the service in the EIP as shown below:
...
The **service name** supports a limited uri like syntax, here are some examples

 
| Name | Resolution |
| --- | --- |
| foo | [http://host:port](http://host:port) |
| foo/path | [http://host:port/path](http://host:port/path) |
| foo/path?foo=bar | [http://host:port/path?foo=bar](http://host:port/path?foo=bar) |
...
This service discovery implementation does not query any external services to find out the list of services associated with a named service but keep them in memory. Each service should be provided in the following form:
...
| namespace | `String` | The Kubernetes namespace to use. By default, the namespace’s name is taken from the environment variable KUBERNETES\_MASTER |
...
The service filter has access to the current exchange, which allows you to create service filters comparing service metadata with message content.
