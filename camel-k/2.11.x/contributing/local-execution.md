...
If you want to run a local operator together with `Minikube` you will need an additional step in order to let the local operator push images in the local registry. We need to expose the local registry as described in [this procedure](https://minikube.sigs.k8s.io/docs/handbook/registry/#docker-on-windows):
