...
| `jvm.print-command` | `bool` | Prints the command used the start the JVM in the container logs (default `true`) Deprecated: no longer in use. |
...
| `jvm.ca-cert` | `string` | Path to a PEM-encoded CA certificate file. Example: "/etc/camel/conf.d/\_secrets/my-ca/ca.crt" |
...
| `jvm.ca-cert-password` | `string` | Required when caCert is set. Path to a file containing the truststore password. Example: "/etc/camel/conf.d/\_secrets/truststore-pass/password" |
