...
| `environment.container-meta` | `bool` | Enables injection of `NAMESPACE` and `POD_NAME` environment variables (default `true`) |
| `environment.http-proxy` | `bool` | Propagates the `HTTP_PROXY`, `HTTPS_PROXY` and `NO_PROXY` environment variables (default `true`) |
