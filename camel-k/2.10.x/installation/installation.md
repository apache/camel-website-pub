...
$ kubectl apply -k github.com/apache/camel-k/install/overlays/kubernetes/descoped?ref=v2.10.2 --server-side

You can specify as `ref` parameter the version you’re willing to install (ie, `v2.10.2`). The command above will install a descoped (global) operator in the camel-k namespace. This is the suggested configuration in order to manage Integrations in all namespaces.
