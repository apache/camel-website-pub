...
The last one is the image used in the Camel K Runtime 3.8.1 version (the default one at the time of writing). Make sure to identify in the [camel-k-catalog](https://repo1.maven.org/maven2/org/apache/camel/k/camel-k-catalog/3.8.1/camel-k-catalog-3.8.1-catalog.yaml) (`.spec.runtime.metadata.quarkus.native-builder-image`) the one required in your installation.
...
It requires that the Maven version from where you’re running the scripts (likely your machine) is the same used in the Camel K operator target version (ie, 3.9.14 for Camel K version 2.10.2) - required to enforce reproducible builds. The script will download the correct version if needed.
...
The script is available in [Camel K github repository](https://github.com/apache/camel-k/blob/main/script/offline_dependencies.sh).
...
You can download a [template settings.xml](https://github.com/apache/camel-k/blob/main/script/) and change it accordingly to the following script, then create a ConfigMap from the settings.xml file.
