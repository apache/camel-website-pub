...
It requires that the Maven version from where you’re running the scripts (likely your machine) is the same used in the Camel K operator target version (ie, 3.9.14 for Camel K version 2.9.1) - required to enforce reproducible builds. The script will download the correct version if needed.
