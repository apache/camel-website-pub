...
There is a [prune-camel-k-kit-images.sh](https://github.com/apache/camel-k/blob/main/script/prune-camel-k-kit-images.sh) script to help you in this task. This script requires the following cli tools: `kubectl, comm, docker`. The script lists the dangling images from the container registry, it accepts two parameters with no arguments: `-v` (verbose) and `-p` (prune images).
