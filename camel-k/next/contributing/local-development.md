...
> **Note**
> you can also run and debug the operator as a local process: see [Operator - local execution](local-execution.md)
...
Camel K integrations are based on [Camel K runtime](https://github.com/apache/camel-k-runtime), generally paired with the operator release. If you need to specify a different runtime, or you have a local Camel K Runtime that you want to test you need to build a Camel K version bundled with the local Camel K Runtime bits

make images CAMEL\_K\_RUNTIME\_DIR=<path/to/local/ck-runtime/project>
