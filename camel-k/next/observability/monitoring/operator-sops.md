...
> **Note**
> The following guide uses the terminology from the [Site Reliability Engineer](https://sre.google/sre-book/service-level-objectives/) book.

The Camel K operator exposes a monitoring endpoint, that publishes [metrics](operator.html#metrics) indicating the _level of service_ provided to its users. These metrics materialize the Service Level Indicators (SLIs) for the Camel K operator.

Service Level Objectives (SLOs) can be defined based on these SLIs. The [default alerts](operator.html#alerting) created for the Camel K operator query the SLIs corresponding metrics, and match the SLOs for the Camel K operator, so that they fire up as soon as the _level of service_ is not met, and preemptive measures can be taken before beaching the Service Level Agreement (SLA) for the Camel K operator.

## Operator SOPs

The following section lists the Standard Operating Procedures (SOPs), corresponding to the [default alerts](operator.html#alerting), created for the Camel K operator. It assumes the operator has been installed according to the [installation](operator.html#installation) section from the operator monitoring documentation.
...
> **Note**
> The commands in the following section rely on the `jq` tool, to process the output of the `kubectl` commands. You can refer to the [download](https://stedolan.github.io/jq/download/) instructions from the tool website.
