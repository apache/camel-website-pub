...
> **Note**
> use the `quarkus.log.level` property to change the level of log, if needed (ie, `kamel run test.yaml -p quarkus.log.level=DEBUG`).
...
If you’re still in trouble, another resource that can help you understand a little better the situation of your configuration is `CamelCatalog` (`kubectl get CamelCatalog`). If they are in phase error, for any reason, you will discover that looking at their status.
