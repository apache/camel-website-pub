...
The document id is read from the `CamelLangChain4jIngestDocumentId` exchange property when a prior step captured it (the parser actions do, before the parse), else from the header named by `documentIdHeader` - the property always wins, so a parsed document cannot forge its own identity. Bean references are `#bean:name` values; when `embeddingStore` or `embeddingModel` is not set, the single registry bean of that type is used. With `idempotentRepository` set, the first write per document id wins - a re-delivered edited document is skipped, so streams that carry updates need a version-aware id.
